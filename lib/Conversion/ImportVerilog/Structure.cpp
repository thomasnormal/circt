...
struct ModuleVisitor : public BaseVisitor {
  ...
  LogicalResult visit(const slang::ast::InstanceSymbol &instNode) {
    ...
    // Record instance's results generated by hierarchical interface paths.
    for (const auto &hierIface : context.hierInterfacePaths[&instNode.body]) {
      if (!hierIface.idx ||
          hierIface.direction != ArgumentDirection::Out ||
          !hierIface.ifaceInst)
        continue;
      Value result = inst->getResult(*hierIface.idx);
      context.interfaceInstances[hierIface.ifaceInst] = result;
      context.threadedInterfaceInstances.insert(hierIface.ifaceInst);
    }
...
Context::convertModuleHeader(const slang::ast::InstanceBodySymbol *module) {
  ...
  for (auto &hierIface : hierInterfacePaths[module]) {
    ...
    if (hierIface.direction == ArgumentDirection::Out) {
      hierIface.idx = outputIdx++;
      modulePorts.push_back({portName, portType, hw::ModulePort::Output});
    } else {
...
Context::convertModuleBody(const slang::ast::InstanceBodySymbol *module) {
  ...
  auto prevThreadedInterfaceInstances = std::move(threadedInterfaceInstances);
  threadedInterfaceInstances.clear();
  auto threadedInterfaceInstancesGuard = llvm::make_scope_exit([&] {
    threadedInterfaceInstances = std::move(prevThreadedInterfaceInstances);
  });
  ...
  for (const auto &hierIface : hierInterfacePaths[module]) {
    if (hierIface.direction != slang::ast::ArgumentDirection::In)
      continue;
  ...
  for (const auto &hierIface : hierInterfacePaths[module]) {
    if (hierIface.direction != slang::ast::ArgumentDirection::Out)
      continue;
...
Value Context::resolveInterfaceInstance(
  ...
  if (shouldCacheInterfaceInstance(*instSym)) {
    if (auto it = interfaceInstances.find(instSym);
      ...
      if (threadedInterfaceInstances.contains(instSym))
        return it->second;
    }
