...
vpiHandle vpi_register_cb(p_cb_data cb_data_p) {
  ...
  // (WASM/JS callers register it before callMain to receive the startup event).
  // Other callback types require active=true because they need the scheduler.
  if (!vpi.isActive() && (!cb_data_p || cb_data_p->reason != cbStartOfSimulation))
    return nullptr;
