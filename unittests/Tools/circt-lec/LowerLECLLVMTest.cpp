...
TEST(LowerLECLLVMTest, LowersCombMuxCastStruct) {
  MLIRContext context;
  context.loadDialect<circt::comb::CombDialect, circt::hw::HWDialect,
                      LLVM::LLVMDialect>();
...
TEST(StripLLHDInterfaceSignalsTest, ResolvesOverlappingStores2State) {
  MLIRContext context;
  context.loadDialect<circt::comb::CombDialect, circt::hw::HWDialect,
                      circt::llhd::LLHDDialect, circt::verif::VerifDialect,
                      LLVM::LLVMDialect, scf::SCFDialect>();
  ...
  auto top = module->lookupSymbol<circt::hw::HWModuleOp>("top");
  ASSERT_TRUE(top);
  EXPECT_EQ(top.getNumInputPorts(), 3u);
...
TEST(LowerLECLLVMTest, LowersBlockArgStructJoin) {
  MLIRContext context;
  context.loadDialect<circt::hw::HWDialect, circt::llhd::LLHDDialect,
                      LLVM::LLVMDialect, mlir::cf::ControlFlowDialect>();
...
TEST(LowerLECLLVMTest, LowersMixedAllocaJoin) {
  MLIRContext context;
  context.loadDialect<circt::hw::HWDialect, circt::llhd::LLHDDialect,
                      LLVM::LLVMDialect, mlir::cf::ControlFlowDialect>();
