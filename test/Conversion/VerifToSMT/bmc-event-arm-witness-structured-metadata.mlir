...
func.func @bmc_event_arm_witness_structured_metadata() {
  %bmc = verif.bmc bound 1 num_regs 0 initial_values [] attributes {
    ...
    bmc_event_sources = [["signal[0]:both:iff", "signal[1]:both"]],
    bmc_event_source_details = [[{edge = "both", iff_name = "en", kind = "signal", label = "signal[0]:both:iff", signal_index = 0 : i32, signal_lsb = 1 : i32, signal_msb = 1 : i32, signal_name = "bus"}, {edge = "both", kind = "signal", label = "signal[1]:both", signal_index = 1 : i32, signal_name = "bus", signal_reduction = "or"}]]
  }
  init {
    verif.yield
  }
