...
// Test derived clock mapping when assume enable is a constant XOR expression.

// CHECK-LABEL: func.func @bmc_derived_clock_assume_enable_xor_const() -> i1
// CHECK: scf.for
// CHECK: smt.ite
func.func @bmc_derived_clock_assume_enable_xor_const() -> i1 {
  ...
  ^bb0(%clk: !seq.clock, %drv: i1, %sig: i1):
    ...
    %eq = comb.icmp eq %clk_i1, %drv : i1
    %c0 = hw.constant false
    %c1 = hw.constant true
    %en = comb.xor %c0, %c1 : i1
    verif.assume %eq if %en : i1
