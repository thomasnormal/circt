...
// Test derived clock mapping via wildcard equality (comb.icmp weq).

// CHECK-LABEL: func.func @bmc_derived_clock_weq() -> i1
// CHECK: scf.for
// CHECK: smt.ite {{%.+}}, {{%.+}}, {{%.+}} : !smt.bv<1>
func.func @bmc_derived_clock_weq() -> i1 {
  ...
  ^bb0(%clk: !seq.clock, %drv: i1, %sig: i1):
    ...
    %eq = comb.icmp weq %clk_i1, %drv : i1
    verif.assume %eq : i1
