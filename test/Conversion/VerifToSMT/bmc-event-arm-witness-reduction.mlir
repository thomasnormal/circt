...
func.func @bmc_event_arm_witness_reduction_expr() {
  %bmc = verif.bmc bound 1 num_regs 0 initial_values [] attributes {
    ...
    bmc_event_sources = [["signal[0]:both:iff_expr", "signal[1]:both"]],
    bmc_event_source_details = [[{edge = "both", iff_expr = "&bus", kind = "signal", label = "signal[0]:both:iff_expr", signal_expr = "|bus", signal_index = 0 : i32}, {edge = "both", kind = "signal", label = "signal[1]:both", signal_expr = "^bus", signal_index = 1 : i32}]]
  }
  init {
    verif.yield
  }
