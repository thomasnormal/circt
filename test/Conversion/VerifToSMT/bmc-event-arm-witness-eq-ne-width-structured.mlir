...
func.func @bmc_event_arm_witness_eq_ne_width_structured() {
  %bmc = verif.bmc bound 1 num_regs 0 initial_values [] attributes {
    bmc_input_names = ["bus", "mask", "en"],
    bmc_event_sources = [["signal[0]:both:iff"]],
    bmc_event_source_details = [[{edge = "both", iff_bin_op = "ne", iff_lhs_name = "bus", iff_rhs_name = "mask", kind = "signal", label = "signal[0]:both:iff", signal_bin_op = "eq", signal_index = 0 : i32, signal_lhs_name = "bus", signal_rhs_name = "mask"}]]
  }
  init {
    verif.yield
  }
