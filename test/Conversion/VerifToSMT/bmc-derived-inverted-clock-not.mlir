...
// Test derived clock mapping through assume when the derived clock is inverted
// using comb.not relative to the primary BMC clock. A posedge on the derived
// clock should gate delay buffers on the base clock negedge.

// CHECK-LABEL: func.func @bmc_derived_inverted_clock_not() -> i1
// CHECK: scf.for
...
func.func @bmc_derived_inverted_clock_not() -> i1 {
  ...
  ^bb0(%clk: !seq.clock, %drv: i1, %sig: i1):
    ...
    %true_inv = hw.constant true
    %inv = comb.xor %clk_i1, %true_inv : i1
    %eq = comb.icmp eq %inv, %drv : i1
    verif.assume %eq : i1
