...
// Test that rising-clocks-only mode handles negedge properties.
// The clock edge information is incorporated into bmc_circuit's return.

// CHECK-LABEL: func.func @bmc_rising_clocks_only_negedge() -> i1
// CHECK:   scf.for
// CHECK:     func.call @bmc_loop
// CHECK:     func.call @bmc_circuit
// CHECK-SAME: -> (!smt.bv<1>, !smt.bool)
func.func @bmc_rising_clocks_only_negedge() -> i1 {
  ...
  ^bb0(%clk: !seq.clock):
    ...
    verif.assert %seq {bmc.clock_edge = #ltl<clock_edge negedge>} : !ltl.sequence
    verif.yield %true : i1
  }
  func.return %bmc : i1
}
