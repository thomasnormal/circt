...
// SMTLIB-DAG: signal_lhs_concat_arity = 2 : i32
// SMTLIB-DAG: iff_bin_op = "ne"
// SMTLIB-DAG: iff_lhs_replicate_count = 2 : i32
...
// SMTLIB-DAG: smt.bv.repeat 2 times
// SMTLIB-DAG: smt.eq {{.*}} : !smt.bv<2>
// SMTLIB-DAG: smt.distinct {{.*}} : !smt.bv<2>

// RUNTIME: smt.solver
// RUNTIME-DAG: signal_bin_op = "eq"
// RUNTIME-DAG: signal_lhs_concat_arity = 2 : i32
// RUNTIME-DAG: iff_bin_op = "ne"
// RUNTIME-DAG: iff_lhs_replicate_count = 2 : i32
...
// RUNTIME-DAG: smt.bv.repeat 2 times
// RUNTIME-DAG: smt.eq {{.*}} : !smt.bv<2>
// RUNTIME-DAG: smt.distinct {{.*}} : !smt.bv<2>

func.func @bmc_event_arm_witness_concat_replicate_structured() {
  %bmc = verif.bmc bound 1 num_regs 0 initial_values [] attributes {
    bmc_input_names = ["a", "b", "ab", "en", "mask", "ok"],
    bmc_event_sources = [["signal[0]:both:iff"]],
    bmc_event_source_details = [[{edge = "both", iff_bin_op = "ne", iff_lhs_arg_name = "en", iff_lhs_replicate_count = 2 : i32, iff_rhs_name = "mask", kind = "signal", label = "signal[0]:both:iff", signal_bin_op = "eq", signal_index = 0 : i32, signal_lhs_cat0_name = "a", signal_lhs_cat1_name = "b", signal_lhs_concat_arity = 2 : i32, signal_rhs_name = "ab"}]]
  }
  init {
    verif.yield
  }
  ...
  circuit {
  ^bb0(%a: i1, %b: i1, %ab: i2, %en: i1, %mask: i2, %ok: i1):
    verif.assert %ok : i1
    verif.yield
  }
  return
