...
// Regression: call_indirect must not entry-dispatch MAY_YIELD functions even
// when the entry is trampoline-only (non-native). Trampoline entries still
// cannot suspend/resume process state correctly for MAY_YIELD semantics.
//
...
// RUNTIME: Entry-table skipped (yield):      1
// RUNTIME: out=1{{$}}
...
func.func @"uvm_pkg::dummy::tagbit_may_yield"(%arg0: !llvm.ptr) -> i64 {
  %run = hw.constant true
  %never = hw.constant false
  cf.cond_br %run, ^live, ^dead_pause

^dead_pause:
  // Ensure this symbol is trampoline-demoted.
  sim.pause quiet
  cf.br ^dead_may_yield

^dead_may_yield:
  // Conservative MAY_YIELD classification trigger (never executed).
  %zero = llvm.mlir.constant(0 : i64) : i64
  %null = llvm.inttoptr %zero : i64 to !llvm.ptr
  %fn = builtin.unrealized_conversion_cast %null : !llvm.ptr to (i64) -> i64
  %dead = func.call_indirect %fn(%zero) : (i64) -> i64
  cf.br ^live
...
^dead:
  // Keep caller interpreted so runtime call_indirect path is exercised.
  sim.pause quiet
  cf.br ^live
...
llvm.mlir.global internal @"uvm_pkg::__vtable__"(#llvm.zero) {
  ...
  circt.vtable_entries = [
    [0, @"uvm_pkg::dummy::tagbit_may_yield"]
  ]
} : !llvm.array<1 x ptr>
