...
// Regression: casting an array element sub-reference to !llvm.ptr in a callee
// must preserve LLVM-side element mapping for memory-backed refs.

// CHECK: head=99 arr0=17 arr1=34 tail=77

module {
  func.func private @store_byte_via_ptr(%elem_ref: !llhd.ref<i8>, %value: i8) {
    %ptr = builtin.unrealized_conversion_cast %elem_ref : !llhd.ref<i8> to !llvm.ptr
  ...
  hw.module @test() {
    ...
    ^bb1:
      ...
      %arr_ref = llhd.sig.struct_extract %obj_ref["arr"] : <!hw.struct<head: i8, arr: !hw.array<2xi8>, tail: i8>>
      %elem0 = llhd.sig.array_get %arr_ref[%c0_i1] : <!hw.array<2xi8>>
      %elem1 = llhd.sig.array_get %arr_ref[%c1_i1] : <!hw.array<2xi8>>

      func.call @store_byte_via_ptr(%elem0, %c17_i8) : (!llhd.ref<i8>, i8) -> ()
      func.call @store_byte_via_ptr(%elem1, %c34_i8) : (!llhd.ref<i8>, i8) -> ()
