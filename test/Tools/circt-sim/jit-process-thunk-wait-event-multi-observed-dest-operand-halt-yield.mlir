...
//
// LOG: jit-process-thunk-wait-event-multi-observed-dest-operand-halt-yield
...
hw.module @top() {
  ...
  %fmt = sim.fmt.literal "jit-process-thunk-wait-event-multi-observed-dest-operand-halt-yield\0A"
  ...
  // Process 2: multi-observed event wait with process-result operands.
  %proc_val = llhd.process -> i1 {
    %val_clk = llhd.prb %clk : i1
    %val_static = llhd.prb %sig_static : i1
    llhd.wait yield (%val_clk : i1), (%val_clk, %val_static : i1, i1), ^bb1(%val_clk : i1)
  ^bb1(%v: i1):
    sim.proc.print %fmt
    llhd.halt %v : i1
  }
