// RUN: env CIRCT_SIM_TRACE_CONFIG_DB=1 circt-sim %s --top test 2>&1 | tee %t.log | FileCheck %s
// RUN: test $(grep -c "CFG-CI-CANON-GET" %t.log) -le 10
//
// Regression: context-name recovery for canonical config_db get() must guard
// against get_full_name() re-entering config_db::get on the same process.
// Without a reentrancy guard this can recurse hundreds of times and eventually
// trigger deep call-indirect stacks in larger UVM designs.
//
// CHECK: get_ok=1
// CHECK: dst_eq_src=1
...
module {
  ...
  llvm.mlir.global internal constant @inst_ctx("ctx\00") {addr_space = 0 : i32}
  ...
  // Reproducer: context-name lookup calls get_full_name(), and this method
  // itself performs config_db::get on the same context.
  func.func @"uvm_pkg::uvm_component::get_full_name"(%self: !llvm.ptr)
      -> !llvm.struct<(ptr, i64)> {
    ...
    %ctx_ptr = llvm.mlir.addressof @inst_ctx : !llvm.ptr
    ...
    %vptr_field = llvm.getelementptr %self[0, 1] : (!llvm.ptr) -> !llvm.ptr, !llvm.struct<"cfg_impl", (i32, ptr)>
    ...
    %null_ref = builtin.unrealized_conversion_cast %null : !llvm.ptr to !llhd.ref<!llvm.ptr>
    %_ok = func.call_indirect %get_fn(%self, %self, %empty_str, %field, %null_ref)
        : (!llvm.ptr, !llvm.ptr, !llvm.struct<(ptr, i64)>, !llvm.struct<(ptr, i64)>, !llhd.ref<!llvm.ptr>) -> i1

    %ctx_s0 = llvm.insertvalue %ctx_ptr, %undef_str[0] : !llvm.struct<(ptr, i64)>
    %ctx = llvm.insertvalue %c3_i64, %ctx_s0[1] : !llvm.struct<(ptr, i64)>
    return %ctx : !llvm.struct<(ptr, i64)>
  }
  ...
  hw.module @test() {
    ...
    %inst_s0 = llvm.insertvalue %ctx_ptr, %undef_str[0] : !llvm.struct<(ptr, i64)>
    %inst_ctx = llvm.insertvalue %c3_i64, %inst_s0[1] : !llvm.struct<(ptr, i64)>
    ...
    ^start:
      ...
      func.call_indirect %set_fn(%obj, %empty_str, %inst_ctx, %field, %src)
      ...
      %ok = func.call_indirect %get_fn(%obj, %obj, %empty_str, %field, %dst_ref)
          : (!llvm.ptr, !llvm.ptr, !llvm.struct<(ptr, i64)>, !llvm.struct<(ptr, i64)>, !llhd.ref<!llvm.ptr>) -> i1
