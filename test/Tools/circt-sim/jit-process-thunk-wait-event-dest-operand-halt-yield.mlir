...
hw.module @top() {
  ...
  %fmt = sim.fmt.literal "jit-process-thunk-wait-event-dest-operand-halt-yield\0A"
  ...
  // Process 2: event-sensitive wait thunk with process-result operands.
  %proc_val = llhd.process -> i1 {
    %val = llhd.prb %clk : i1
    llhd.wait yield (%val : i1), (%val : i1), ^bb1(%val : i1)
  ^bb1(%v: i1):
    sim.proc.print %fmt
    llhd.halt %v : i1
  }
