...
// RUN: env CIRCT_AOT_ALLOW_NATIVE_UVM_ACCESSORS=1 circt-sim-compile %s -o %t.optin.so 2>&1 | FileCheck %s --check-prefix=COMPILE-OPTIN
// RUN: env CIRCT_AOT_ALLOW_NATIVE_UVM_ACCESSORS=1 circt-sim %s --compiled=%t.optin.so 2>&1 | FileCheck %s --check-prefix=OPTIN

// Opt-in regression: allow native dispatch/compilation of UVM accessor symbols.
//
...
func.func @"uvm_pkg::toy_pull_port::get_imp_123"(%a: i32) -> i32 {
  %c42 = arith.constant 42 : i32
  %r = arith.addi %a, %c42 : i32
  return %r : i32
}

// Keep one non-intercepted function compiled in default mode so .so emission
// still succeeds when get_imp_123 is demoted.
...
hw.module @test() {
  ...
  ^print:
    %r = func.call @"uvm_pkg::toy_pull_port::get_imp_123"(%c5_i32) : (i32) -> i32
