...
//
// LOG: jit-process-thunk-wait-event-multi-derived-observed-dest-operand-halt-yield
...
hw.module @top() {
  ...
  %sig_static = llhd.sig %false : i1
  %sig_out = llhd.sig %false : i1
  %fmt = sim.fmt.literal "jit-process-thunk-wait-event-multi-derived-observed-dest-operand-halt-yield\0A"
  ...
  // Both observed values are derived in the pre-wait prelude.
  %proc_val = llhd.process -> i1 {
    %val_clk = llhd.prb %clk : i1
    %val_static = llhd.prb %sig_static : i1
    %inv_clk = comb.xor %val_clk, %true : i1
    %gate = comb.or %val_clk, %val_static : i1
    llhd.wait yield (%gate : i1), (%inv_clk, %gate : i1, i1), ^bb1(%gate : i1)
  ^bb1(%v: i1):
    sim.proc.print %fmt
    llhd.halt %v : i1
  }
