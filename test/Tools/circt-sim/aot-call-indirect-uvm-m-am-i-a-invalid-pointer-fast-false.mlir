...
// Regression: call_indirect native dispatch into
// uvm_pkg::uvm_typed_callbacks_*::m_am_i_a must not segfault when the rhs
// pointer payload is malformed/non-resident. The runtime should short-circuit
// the predicate to false instead of native dereference.
//
...
^bb_null:
  return %true : i1
^bb_deref:
  // Native execution of this path with malformed %rhs crashes if call_indirect
  // does not guard unsafe pointer payloads.
...
^dead:
  // Force this call site onto interpreted call_indirect path.
  sim.pause quiet
  cf.br ^live
^live:
...
hw.module @top() {
  ...
  llhd.process {
    ...
    // Corrupted high pointer payload seen in MAX_NATIVE stress traces.
