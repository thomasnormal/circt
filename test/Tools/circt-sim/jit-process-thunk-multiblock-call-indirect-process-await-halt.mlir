...
module {
  ...
  func.func @callee_wait(%target: !llvm.ptr) {
  ...
  llvm.mlir.global internal @"cls::__vtable__"(#llvm.zero) {
    addr_space = 0 : i32,
    circt.vtable_entries = [[0, @callee_wait]]
  } : !llvm.array<1 x ptr>
  ...
  hw.module @top() {
    ...
    ^bb1:
      ...
      %fn = builtin.unrealized_conversion_cast %fnptr : !llvm.ptr to (!llvm.ptr) -> ()
      func.call_indirect %fn(%target) : (!llvm.ptr) -> ()
      cf.br ^bb2
    ^bb2:
