// RUN: rm -rf %t
// RUN: mkdir -p %t
// RUN: env CIRCT_SIM_TRACE_JIT_THUNK_GUARDS=1 circt-sim %s --skip-passes --mode=compile --jit-hot-threshold=1 --jit-compile-budget=64 --max-time=100 --jit-report=%t/jit.json > %t/log.txt 2>&1
// RUN: FileCheck %s --check-prefix=LOG < %t/log.txt
// RUN: FileCheck %s --check-prefix=JSON < %t/jit.json
//
// LOG: [JIT-THUNK-GUARD] proc={{[0-9]+}} name=llhd_process_{{[0-9]+}} shape=indirect_target_set_guard reason=call_indirect_target_guard:target_set_changed:site=
// LOG: profile_guided_indirect_guard_mismatch_done
...
// JSON-DAG: "target_set_version": 2
// JSON-DAG: "targets_total": 2
// JSON-DAG: "target_name": "callee_a"
// JSON-DAG: "target_name": "callee_b"
...
module {
  ...
  hw.module @top() {
    %fmt = sim.fmt.literal "profile_guided_indirect_guard_mismatch_done\0A"
    ...
    ^bb1:
      ...
      // First activation runs slot 0 (callee_a). Later activations run slot 1.
      %slot_one = arith.constant 1 : i32
      llvm.store %slot_one, %slot_addr : i32, !llvm.ptr
