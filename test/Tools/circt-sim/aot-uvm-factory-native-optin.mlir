...
// RUN: env CIRCT_AOT_ALLOW_NATIVE_UVM_FACTORY=1 circt-sim-compile %s -o %t.optin.so 2>&1 | FileCheck %s --check-prefix=COMPILE-OPTIN
// RUN: env CIRCT_AOT_ALLOW_NATIVE_UVM_FACTORY=1 circt-sim %s --compiled=%t.optin.so 2>&1 | FileCheck %s --check-prefix=OPTIN

// Opt-in regression: keep uvm_factory paths interpreted by default for
// stability, with explicit native enable for experiments.
//
...
func.func @"uvm_pkg::uvm_factory::register"(%a: i32, %b: i32) -> i32 {
...
hw.module @test() {
  ...
  ^print:
    %r = func.call @"uvm_pkg::uvm_factory::register"(%c10_i32, %c20_i32) : (i32, i32) -> i32
