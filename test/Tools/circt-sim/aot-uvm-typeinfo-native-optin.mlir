...
// RUN: env CIRCT_AOT_ALLOW_NATIVE_UVM_TYPEINFO=1 circt-sim-compile %s -o %t.optin.so 2>&1 | FileCheck %s --check-prefix=COMPILE-OPTIN
// RUN: env CIRCT_AOT_ALLOW_NATIVE_UVM_TYPEINFO=1 circt-sim %s --compiled=%t.optin.so 2>&1 | FileCheck %s --check-prefix=OPTIN

// Opt-in regression: allow native dispatch/compilation of UVM type-info paths.
//
...
// still succeeds when get_type_name is demoted.
