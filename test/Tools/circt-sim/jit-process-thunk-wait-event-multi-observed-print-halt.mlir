...
//
// LOG: jit-process-thunk-wait-event-multi-observed-print-halt
...
hw.module @top() {
  ...
  %fmt = sim.fmt.literal "jit-process-thunk-wait-event-multi-observed-print-halt\0A"
  ...
  // Process 2: multi-observed event wait thunk.
  llhd.process {
    %val_clk = llhd.prb %clk : i1
    %val_static = llhd.prb %sig_static : i1
    llhd.wait (%val_clk, %val_static : i1, i1), ^bb1
  ^bb1:
    sim.proc.print %fmt
    llhd.halt
  }
