...
// RUN: env CIRCT_AOT_ALLOW_NATIVE_UVM_ALLOC=1 circt-sim-compile %s -o %t.optin.so 2>&1 | FileCheck %s --check-prefix=COMPILE-OPTIN
// RUN: env CIRCT_AOT_ALLOW_NATIVE_UVM_ALLOC=1 circt-sim %s --compiled=%t.optin.so 2>&1 | FileCheck %s --check-prefix=OPTIN

// Opt-in regression: allow native dispatch/compilation of UVM allocation paths.
//
...
hw.module @test() {
  ...
  ^print:
    %r = func.call @"uvm_pkg::toy_object::new"(%c10_i32, %c20_i32) : (i32, i32) -> i32
