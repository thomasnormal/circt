...
// uvm_pkg::uvm_typed_callbacks_*::m_am_i_a must short-circuit to false for
// medium-range unmapped rhs payloads (not just very low/high addresses).
//
...
^bb_null:
  return %true : i1
^bb_deref:
  // Native execution with malformed/unmapped %rhs will crash without
  // call_indirect arg1 safety checks.
...
hw.module @top() {
  ...
  llhd.process {
    ...
    // Mid-range, non-canonical UVM-handle payload observed in APB stress runs.
    // 0x6620fffe
    %rhs_i64 = llvm.mlir.constant(1713438718 : i64) : i64
