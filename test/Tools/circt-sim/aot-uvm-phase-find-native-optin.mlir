...
// RUN: env CIRCT_AOT_ALLOW_NATIVE_UVM_PHASE_GRAPH=1 circt-compile %s -o %t.optin.so 2>&1 | FileCheck %s --check-prefix=COMPILE-OPTIN
// RUN: env CIRCT_AOT_ALLOW_NATIVE_UVM_PHASE_GRAPH=1 circt-sim %s --compiled=%t.optin.so 2>&1 | FileCheck %s --check-prefix=OPTIN

// Regression: keep UVM phase-graph search helpers interpreted by default.
//
...
func.func @"uvm_pkg::uvm_phase::m_find_successor"(%a: i32, %b: i32) -> i32 {
...
hw.module @test() {
  ...
  ^print:
    %r = func.call @"uvm_pkg::uvm_phase::m_find_successor"(%c42_i32, %c11_i32) : (i32, i32) -> i32
