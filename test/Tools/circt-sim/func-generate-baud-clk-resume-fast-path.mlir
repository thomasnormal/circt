// RUN: env CIRCT_SIM_TRACE_BAUD_FASTPATH=1 circt-sim %s --max-time=1000000 --max-process-steps=260 2>&1 | FileCheck %s
// RUN: env CIRCT_SIM_TRACE_BAUD_FASTPATH=1 circt-sim %s --parallel=4 --work-stealing --auto-partition --max-time=1000000 --max-process-steps=260 2>&1 | FileCheck %s

// This regression locks the caller-side resume fast path for
// `*::GenerateBaudClk` wrappers that tail-call `*::BaudClkGenerator`.
//
// CHECK: [BAUD-GEN-FP] resume-hit
// CHECK: out=
...
module {
  ...
  func.func private @"Toy::GenerateBaudClk"(%arg0: !llvm.ptr) {
    ...
    func.call @"Toy::BaudClkGenerator"(%arg0, %c2_i32) : (!llvm.ptr, i32) -> ()
    return
  }
  ...
  hw.module @test() {
    ...
    // Caller-side wrapper under test.
    llhd.process {
      %start = llhd.int_to_time %c1_i64
      llhd.wait delay %start, ^entry
    ^entry:
      %gptr = llvm.mlir.addressof @"toy::obj_ptr" : !llvm.ptr
      %obj = llvm.load %gptr : !llvm.ptr -> !llvm.ptr
      func.call @"Toy::GenerateBaudClk"(%obj) : (!llvm.ptr) -> ()
      llhd.halt
    }
