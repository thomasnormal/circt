// RUN: env CIRCT_SIM_TRACE_BAUD_FASTPATH=1 CIRCT_SIM_BAUD_FASTPATH_DELAY_BATCH_MIN_STABLE_EDGES=2 CIRCT_SIM_BAUD_FASTPATH_DELAY_BATCH_MAX_EDGES=8 circt-sim %s --max-time=1000000 --max-process-steps=300 2>&1 | FileCheck %s
//
// CHECK: [BAUD-FP] batch-schedule
// CHECK: out=1
...
module {
  ...
  hw.module @test() {
    ...
    ^entry:
      %gptr = llvm.mlir.addressof @"toy::obj_ptr" : !llvm.ptr
      %obj = llvm.load %gptr : !llvm.ptr -> !llvm.ptr
      func.call @"Toy::BaudClkGenerator"(%obj, %c4_i32) : (!llvm.ptr, i32) -> ()
      llhd.halt
    }

    llhd.process {
      %check_delay = llhd.int_to_time %c220_i64
