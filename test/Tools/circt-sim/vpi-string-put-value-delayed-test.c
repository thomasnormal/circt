// VPI string put_value delayed test for signal-backed string ports.
// Writes a string via cbAfterDelay and checks the HDL-derived ASCII sum
// on a later callback.
...
static const char *teststr = "\x1b[33myellow\x1b[49m\x1b[39m";
...
static PLI_INT32 write_cb(struct t_cb_data *cb_data) {
  (void)cb_data;
  vpiHandle strSig =
      vpi_handle_by_name("vpi_string_delay_test.stream_in_string", NULL);
...
static PLI_INT32 read_cb(struct t_cb_data *cb_data) {
  (void)cb_data;
  const int expected_sum = 1668;

  vpiHandle strSig =
      vpi_handle_by_name("vpi_string_delay_test.stream_in_string", NULL);
  vpiHandle sumSig = vpi_handle_by_name(
      "vpi_string_delay_test.stream_in_string_asciival_sum", NULL);
  CHECK(strSig != NULL, "found stream_in_string");
  ...
  fprintf(stderr, "VPI_STRING_DELAY: asciival_sum=%d\n", readSum.value.integer);
  CHECK(readSum.value.integer == expected_sum, "ascii sum updates from string");

  fprintf(stderr, "VPI_STRING_DELAY: %d passed, %d failed\n", tests_passed,
          tests_failed);
  return 0;
}
...
static PLI_INT32 start_of_sim_cb(struct t_cb_data *cb_data) {
  (void)cb_data;
  register_after_delay(/*delayPs=*/10, write_cb);
  register_after_delay(/*delayPs=*/20, read_cb);
  return 0;
}

static PLI_INT32 end_of_sim_cb(struct t_cb_data *cb_data) {
  (void)cb_data;
  fprintf(stderr, "VPI_STRING_DELAY: FINAL: %d passed, %d failed\n",
