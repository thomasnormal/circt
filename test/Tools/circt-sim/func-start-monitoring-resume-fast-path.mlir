// RUN: env CIRCT_SIM_TRACE_MONITOR_DESERIALIZER_FASTPATH=1 circt-sim %s --max-time=1000000 --max-process-steps=260 2>&1 | FileCheck %s
// RUN: env CIRCT_SIM_TRACE_MONITOR_DESERIALIZER_FASTPATH=1 circt-sim %s --parallel=4 --work-stealing --auto-partition --max-time=1000000 --max-process-steps=260 2>&1 | FileCheck %s
...
module {
  ...
  func.func private @"Toy::StartMonitoring"(%arg0: !llvm.ptr) {
    call @"Toy::Deserializer"(%arg0) : (!llvm.ptr) -> ()
    return
  }

  func.func private @"Toy::Deserializer"(%arg0: !llvm.ptr) {
  ...
  hw.module @test() {
    ...
    ^entry:
      %gptr = llvm.mlir.addressof @"toy::obj_ptr" : !llvm.ptr
      %obj = llvm.load %gptr : !llvm.ptr -> !llvm.ptr
      func.call @"Toy::StartMonitoring"(%obj) : (!llvm.ptr) -> ()
      llhd.halt
    }
