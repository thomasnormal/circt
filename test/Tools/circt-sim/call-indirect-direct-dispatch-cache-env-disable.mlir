// RUN: env CIRCT_SIM_DISABLE_CALL_INDIRECT_DIRECT_DISPATCH_CACHE=1 CIRCT_SIM_PROFILE_SUMMARY_AT_EXIT=1 circt-sim %s 2>&1 | FileCheck %s
//
// When the dispatch cache is disabled, the resolution cache should still work.
// The dispatch cache stats should NOT appear.
//
// CHECK: ok
// CHECK: [circt-sim] call_indirect resolution cache: installs=
// CHECK-NOT: [circt-sim] call_indirect direct dispatch cache:
