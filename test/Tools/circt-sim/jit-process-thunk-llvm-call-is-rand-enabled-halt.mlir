// RUN: rm -rf %t
// RUN: mkdir -p %t
// RUN: circt-sim %s --skip-passes --mode=compile --jit-hot-threshold=1 --jit-compile-budget=8 --jit-fail-on-deopt --max-time=1 --jit-report=%t/jit.json > %t/log.txt 2>&1
// RUN: FileCheck %s --check-prefix=LOG < %t/log.txt
// RUN: FileCheck %s --check-prefix=JSON < %t/jit.json
//
// LOG: rand_enabled=1
// LOG: [circt-sim] Simulation completed
//
// JSON: "mode": "compile"
// JSON: "jit":
// JSON: "jit_deopts_total": 0
// JSON: "jit_deopt_reason_guard_failed": 0
// JSON: "jit_deopt_reason_unsupported_operation": 0
// JSON: "jit_deopt_reason_missing_thunk": 0

module {
  llvm.func @__moore_is_rand_enabled(!llvm.ptr, !llvm.ptr) -> i32

  hw.module @top() {
    %null = llvm.mlir.zero : !llvm.ptr
    %prefix = sim.fmt.literal "rand_enabled="
    %nl = sim.fmt.literal "\0A"

    llhd.process {
      %mode = llvm.call @__moore_is_rand_enabled(%null, %null) : (!llvm.ptr, !llvm.ptr) -> i32
      %mode_fmt = sim.fmt.dec %mode : i32
      %out = sim.fmt.concat (%prefix, %mode_fmt, %nl)
      sim.proc.print %out
      llhd.halt
    }

    hw.output
  }
}
