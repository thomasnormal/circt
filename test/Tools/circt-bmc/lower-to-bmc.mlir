...
// CHECK:    [[BMC:%.+]] = verif.bmc bound 22 num_regs 0 initial_values []{{.*}} init {
// CHECK:    } loop {
// CHECK:    } circuit {
...
// CHECK:    return
// CHECK:  }
// CHECK:  llvm.mlir.global private constant [[SSTR]]("BMC_RESULT=UNSAT\0ABound reached with no violations!\0A\00") {addr_space = 0 : i32}
// CHECK:  llvm.mlir.global private constant [[FSTR]]("BMC_RESULT=SAT\0AAssertion can be violated!\0A\00") {addr_space = 0 : i32}

// RUN: circt-opt --lower-to-bmc="top-module=comb bound=10 ignore-asserts-until=3" %s | FileCheck %s --check-prefix=CHECKIGNOREUNTIL
// CHECKIGNOREUNTIL:    {{%.+}} = verif.bmc bound 22 num_regs 0 initial_values [] attributes {{{.*}}ignore_asserts_until = 6 : i32{{.*}}} init {
...
// CHECK1:    [[BMC:%.+]] = verif.bmc bound 22 num_regs 1 initial_values [unit]{{.*}} init {
...
// CHECK1:    return
// CHECK1:  }
// CHECK1:  llvm.mlir.global private constant [[SSTR]]("BMC_RESULT=UNSAT\0ABound reached with no violations!\0A\00") {addr_space = 0 : i32}
// CHECK1:  llvm.mlir.global private constant [[FSTR]]("BMC_RESULT=SAT\0AAssertion can be violated!\0A\00") {addr_space = 0 : i32}

// RUN: circt-opt --lower-to-bmc="top-module=seq bound=10 rising-clocks-only=true" %s | FileCheck %s --check-prefix=CHECKRISING
// CHECKRISING:    [[BMC:%.+]] = verif.bmc bound 11 num_regs 1 initial_values [unit]{{.*}} init {
...
// CHECK2:    [[BMC:%.+]] = verif.bmc bound 22 num_regs 1 initial_values [unit]{{.*}} init {
...
// CHECK2:    return
// CHECK2:  }
// CHECK2:  llvm.mlir.global private constant [[SSTR]]("BMC_RESULT=UNSAT\0ABound reached with no violations!\0A\00") {addr_space = 0 : i32}
// CHECK2:  llvm.mlir.global private constant [[FSTR]]("BMC_RESULT=SAT\0AAssertion can be violated!\0A\00") {addr_space = 0 : i32}
