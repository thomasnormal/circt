"""End-to-end tests for the CIRCT Verilog LSP server using pytest-lsp."""

import pathlib

import pytest
import pytest_lsp
from lsprotocol import types
from pytest_lsp import ClientServerConfig, LanguageClient, client_capabilities

# Path to the CIRCT Verilog LSP server
CIRCT_VERILOG_LSP = pathlib.Path(__file__).parents[4] / "build" / "bin" / "circt-verilog-lsp-server"

# Test workspace
TEST_DIR = pathlib.Path(__file__).parent
TEST_FILE = TEST_DIR / "sample.sv"


@pytest_lsp.fixture(
    config=ClientServerConfig(
        server_command=[str(CIRCT_VERILOG_LSP)],
    ),
)
async def client(lsp_client: LanguageClient):
    """Set up the LSP client for testing."""
    params = types.InitializeParams(
        capabilities=client_capabilities("visual-studio-code"),
        root_uri=TEST_DIR.as_uri(),
        workspace_folders=[
            types.WorkspaceFolder(uri=TEST_DIR.as_uri(), name="test"),
        ],
    )
    await lsp_client.initialize_session(params)

    yield

    await lsp_client.shutdown_session()


@pytest.mark.asyncio
async def test_initialize(client: LanguageClient):
    """Test that the server initializes correctly."""
    # The client fixture already does initialization, so if we get here it worked
    assert client.capabilities is not None


@pytest.mark.asyncio
async def test_document_open_and_diagnostics(client: LanguageClient):
    """Test opening a document and receiving diagnostics."""
    uri = TEST_FILE.as_uri()
    content = TEST_FILE.read_text()

    # Open the document
    client.text_document_did_open(
        types.DidOpenTextDocumentParams(
            text_document=types.TextDocumentItem(
                uri=uri,
                language_id="systemverilog",
                version=1,
                text=content,
            )
        )
    )

    # Wait a bit for diagnostics (server may be async)
    import asyncio
    await asyncio.sleep(1)

    # Check diagnostics were received (may be empty if file is valid)
    # The key existing means the server processed the file
    assert uri in client.diagnostics or len(client.diagnostics) == 0


@pytest.mark.asyncio
async def test_document_symbols(client: LanguageClient):
    """Test document symbols request."""
    uri = TEST_FILE.as_uri()
    content = TEST_FILE.read_text()

    # Open the document first
    client.text_document_did_open(
        types.DidOpenTextDocumentParams(
            text_document=types.TextDocumentItem(
                uri=uri,
                language_id="systemverilog",
                version=1,
                text=content,
            )
        )
    )

    import asyncio
    await asyncio.sleep(0.5)

    # Request document symbols
    result = await client.text_document_document_symbol_async(
        types.DocumentSymbolParams(
            text_document=types.TextDocumentIdentifier(uri=uri),
        )
    )

    # Should have symbols for our modules
    assert result is not None
    symbols = result if isinstance(result, list) else []

    # Extract symbol names
    symbol_names = []
    for sym in symbols:
        if isinstance(sym, types.DocumentSymbol):
            symbol_names.append(sym.name)
        elif isinstance(sym, types.SymbolInformation):
            symbol_names.append(sym.name)

    # We expect to find our modules
    assert "counter" in symbol_names or "top" in symbol_names or len(symbols) > 0


@pytest.mark.asyncio
async def test_hover(client: LanguageClient):
    """Test hover information."""
    uri = TEST_FILE.as_uri()
    content = TEST_FILE.read_text()

    # Open the document first
    client.text_document_did_open(
        types.DidOpenTextDocumentParams(
            text_document=types.TextDocumentItem(
                uri=uri,
                language_id="systemverilog",
                version=1,
                text=content,
            )
        )
    )

    import asyncio
    await asyncio.sleep(0.5)

    # Hover over "counter" module instantiation (line 27, "counter" starts at column 4)
    result = await client.text_document_hover_async(
        types.HoverParams(
            text_document=types.TextDocumentIdentifier(uri=uri),
            position=types.Position(line=27, character=5),
        )
    )

    # Hover may return None or a Hover object
    # Just verify the request completed without error
    assert result is None or isinstance(result, types.Hover)


@pytest.mark.asyncio
async def test_goto_definition(client: LanguageClient):
    """Test go to definition."""
    uri = TEST_FILE.as_uri()
    content = TEST_FILE.read_text()

    # Open the document first
    client.text_document_did_open(
        types.DidOpenTextDocumentParams(
            text_document=types.TextDocumentItem(
                uri=uri,
                language_id="systemverilog",
                version=1,
                text=content,
            )
        )
    )

    import asyncio
    await asyncio.sleep(0.5)

    # Go to definition of "counter" module instance (line 27)
    result = await client.text_document_definition_async(
        types.DefinitionParams(
            text_document=types.TextDocumentIdentifier(uri=uri),
            position=types.Position(line=27, character=5),
        )
    )

    # Should get a location or list of locations
    assert result is None or isinstance(result, (types.Location, list))


@pytest.mark.asyncio
async def test_completion(client: LanguageClient):
    """Test code completion."""
    uri = TEST_FILE.as_uri()
    content = TEST_FILE.read_text()

    # Open the document first
    client.text_document_did_open(
        types.DidOpenTextDocumentParams(
            text_document=types.TextDocumentItem(
                uri=uri,
                language_id="systemverilog",
                version=1,
                text=content,
            )
        )
    )

    import asyncio
    await asyncio.sleep(0.5)

    # Request completions inside the top module
    result = await client.text_document_completion_async(
        types.CompletionParams(
            text_document=types.TextDocumentIdentifier(uri=uri),
            position=types.Position(line=25, character=4),
        )
    )

    # May return CompletionList, list, or None
    assert result is None or isinstance(result, (types.CompletionList, list))


@pytest.mark.asyncio
async def test_document_with_error(client: LanguageClient):
    """Test that the server reports diagnostics for invalid code."""
    uri = "file:///tmp/invalid.sv"
    content = """
module broken (
    input logic clk
    // Missing closing paren and semicolon
    output logic out
);
    // Syntax error - missing assignment
    assign out
endmodule
"""

    # Open the document
    client.text_document_did_open(
        types.DidOpenTextDocumentParams(
            text_document=types.TextDocumentItem(
                uri=uri,
                language_id="systemverilog",
                version=1,
                text=content,
            )
        )
    )

    # Wait for diagnostics
    import asyncio
    await asyncio.sleep(1)

    # Should have diagnostics for this invalid file
    if uri in client.diagnostics:
        diagnostics = client.diagnostics[uri]
        # We expect some errors for this invalid syntax
        assert len(diagnostics) > 0


@pytest.mark.asyncio
async def test_find_references(client: LanguageClient):
    """Test find references."""
    uri = TEST_FILE.as_uri()
    content = TEST_FILE.read_text()

    # Open the document first
    client.text_document_did_open(
        types.DidOpenTextDocumentParams(
            text_document=types.TextDocumentItem(
                uri=uri,
                language_id="systemverilog",
                version=1,
                text=content,
            )
        )
    )

    import asyncio
    await asyncio.sleep(0.5)

    # Find references to "enable" signal
    result = await client.text_document_references_async(
        types.ReferenceParams(
            text_document=types.TextDocumentIdentifier(uri=uri),
            position=types.Position(line=24, character=10),  # "enable" declaration
            context=types.ReferenceContext(include_declaration=True),
        )
    )

    # Should get a list/tuple of locations (may be empty)
    assert result is None or isinstance(result, (list, tuple))
