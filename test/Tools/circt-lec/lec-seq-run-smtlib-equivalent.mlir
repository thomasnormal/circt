// REQUIRES: z3
// RUN: circt-lec --run-smtlib --bound 5 -c1=counterA -c2=counterB %s | FileCheck %s

// Two equivalent 8-bit counters with different implementation styles.
...
hw.module @counterB(in %clk: !seq.clock,
                    in %r_state: i8, out out: i8, out r_next: i8)
    attributes {num_regs = 1 : i32, initial_values = [0 : i8]} {
  %one = hw.constant 1 : i8
