...
// CHECK-DAG:      %[[FB:.*]] = sim.fmt.bin %[[ARG]] specifierWidth 32 : i32
// CHECK-DAG:      %[[FD:.*]] = sim.fmt.dec %[[ARG]] specifierWidth 10 : i32
// CHECK-DAG:      %[[FH:.*]] = sim.fmt.hex %[[ARG]], isUpper false specifierWidth 8 : i32
// CHECK-DAG:      %[[FO:.*]] = sim.fmt.oct %[[ARG]]  specifierWidth 11 : i32
...
// CHECK-DAG:      %[[B0:.*]] = sim.fmt.bin %[[ARG0]] specifierWidth 8 : i8
// CHECK-DAG:      %[[H0:.*]] = sim.fmt.hex %[[ARG0]], isUpper false specifierWidth 2 : i8
// CHECK-DAG:      %[[O0:.*]] = sim.fmt.oct %[[ARG0]] specifierWidth 3 : i8
// CHECK-DAG:      %[[B1:.*]] = sim.fmt.bin %[[ARG1]] specifierWidth 8 : i8
// CHECK-DAG:      %[[H1:.*]] = sim.fmt.hex %[[ARG1]], isUpper false specifierWidth 2 : i8
// CHECK-DAG:      %[[O1:.*]] = sim.fmt.oct %[[ARG1]] specifierWidth 3 : i8
// CHECK-DAG:      %[[B2:.*]] = sim.fmt.bin %[[ARG2]] specifierWidth 8 : i8
// CHECK-DAG:      %[[H2:.*]] = sim.fmt.hex %[[ARG2]], isUpper false specifierWidth 2 : i8
// CHECK-DAG:      %[[O2:.*]] = sim.fmt.oct %[[ARG2]] specifierWidth 3 : i8
...
hw.module @multi_args(in %clk : !seq.clock, in %a: i8, in %b: i8, in %c: i8) {
  ...
  %bina = sim.fmt.bin %a specifierWidth 8 : i8
  %binb = sim.fmt.bin %b specifierWidth 8 : i8
  %binc = sim.fmt.bin %c specifierWidth 8 : i8

  %hexa = sim.fmt.hex %a, isUpper false specifierWidth 2 : i8
  %hexb = sim.fmt.hex %b, isUpper false specifierWidth 2 : i8
  %hexc = sim.fmt.hex %c, isUpper false specifierWidth 2 : i8

  %octa = sim.fmt.oct %a specifierWidth 3 : i8
  %octb = sim.fmt.oct %b specifierWidth 3 : i8
  %octc = sim.fmt.oct %c specifierWidth 3 : i8
...
// CHECK-DAG:      %[[FA:.*]] = sim.fmt.hex %[[ARGA]], isUpper false specifierWidth 8 : i32
...
// CHECK-DAG:      %[[FB:.*]] = sim.fmt.hex %[[ARGB]], isUpper false specifierWidth 8 : i32
...
// CHECK-DAG:      %[[FC:.*]] = sim.fmt.hex %[[ARGC]], isUpper false specifierWidth 8 : i32
...
hw.module @sequence(in %clk: !seq.clock, in %conda: i1, in %condb: i1, in %val : i32, in %rval : f64) {
  ...
  %cen = sim.fmt.bin %val specifierWidth 32 : i32
...
// CHECK-NEXT:  ^bb0(%[[ARG:.*]]: i1):
// CHECK-NEXT:    %[[BIN:.*]] = sim.fmt.bin %[[ARG]] specifierWidth 1 : i1
