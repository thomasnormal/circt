...
- **Progress**: `first_match` boundedness checks now treat `[=]`/`[->]` repetition as unbounded (new `errors.sv` regression). Nested `disable iff` lowering now combines disable conditions (LTLToCore regression added).
