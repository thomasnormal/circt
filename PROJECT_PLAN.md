...
| - Native mutation CLI frontend (`circt-mut`) | IN_PROGRESS | — | Added initial `circt-mut` tool with `cover|matrix|generate` subcommands that dispatch to existing mutation scripts; default mutation materialization no longer depends on `~/mcy/scripts/create_mutated.sh` (now uses in-repo `utils/create_mutated_yosys.sh`); migration of shell logic into C++ subcommands is pending |
