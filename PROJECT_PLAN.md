...
- **Progress**: `first_match` boundedness checks now treat `[=]`/`[->]` repetition as unbounded, propagate bounds through `or/and/intersect`, and handle clocked sequences (`ltl.clock`/`ltl.past`) with new `errors.sv` regressions. Nested `disable iff` lowering now combines disable conditions (LTLToCore regression added).
