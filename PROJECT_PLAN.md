...
- **Progress**: `first_match` boundedness checks now treat `[=]`/`[->]` repetition as unbounded and propagate bounds through `or/and/intersect` (new `errors.sv` regressions). Nested `disable iff` lowering now combines disable conditions (LTLToCore regression added).
