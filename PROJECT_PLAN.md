...
9. Latest canonical OpenTitan dual-lane run via `run_formal_all.sh`
   ...
   - `E2E`: `pass=10 fail=2` (`SIM:gpio`, `SIM:uart`)
   - `E2E_STRICT`: `pass=11 fail=1` (`LEC:aes_sbox_canright#XPROP_ONLY`)
   - `E2E_MODE_DIFF`: `strict_only_fail=1 strict_only_pass=2`
...
2. OpenTitan default E2E currently has non-strict-only simulation regressions
   (`SIM:gpio`, `SIM:uart`) while strict E2E passes those targets; current
   `E2E_MODE_DIFF` reports `strict_only_pass=2`.
