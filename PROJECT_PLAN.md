...
- **Progress**: `first_match` boundedness checks now treat `[=]`/`[->]` repetition as unbounded, propagate bounds through `or/and/intersect` (treating unbounded operands as infinite for `and/intersect`), and handle clocked sequences (`ltl.clock`/`ltl.past`) with new `errors.sv` regressions (including a clocked `first_match` case). LTLToCore now derives bounds for `or/and` when operands are bounded (warmup regression added). Nested `disable iff` lowering now combines disable conditions (LTLToCore regression added), ImportVerilog covers nested disable SVA lowering (`sva-disable-iff-nested.sv`), and bounded `within/throughout/intersect` sequence composition has regression coverage.
