...
### Iteration 601
- Yosys SVA BMC context schema dependency constraints:
  - Added per-key dependency metadata:
    - `requires`: list of required sibling/built-in context keys
    - `requires_when_regex`: conditional gate over the current key value
  - Added strict schema validation for dependency metadata:
    - reject missing/empty `requires`
    - reject duplicate/self dependencies
    - reject `requires_when_regex` without `requires`
  - Dependency checks now run with required-key checks and honor merged-context
    mode (`validate_merged_context=true`) so built-in context dependencies can
    be validated at effective-context time.
- Regression tests:
  - Expanded
    `test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test`
    with:
    - positive merged dependency route match
    - missing dependency rejection
    - conditional missing dependency rejection
    - invalid dependency schema rejection
- Validation status:
  - `bash -n utils/run_yosys_sva_circt_bmc.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test` -> 1/1 PASS
  - `build/bin/llvm-lit -sv $(rg --files test/Tools | rg 'run-yosys-sva-bmc-summary-history-drop-events.*\\.test$')` -> 16/16 PASS
  - External smoke sweep:
    ...
    - `utils/run_avip_circt_verilog.sh /home/thomas-ahle/mbit/jtag_avip`
      -> PASS
- Current limitations / debt:
  - No top-level composite schema operators yet (`all_of` / `any_of` /
    cross-key comparisons).
  - Context dependency constraints remain key-local and presence-oriented.
  - OpenTitan LEC still needs `LEC_ACCEPT_XPROP_ONLY=1` for
    `aes_sbox_canright`.
- Long-term features to prioritize:
  - Add top-level composite route-context schema operators and cross-key value
    relations.
  - Add parameterized/importable selector macro libraries to reduce duplicated
    JSON policy boilerplate.
  - Reduce `XPROP_ONLY` dependence in OpenTitan LEC by improving 4-state/xprop
    alignment and unknown initialization semantics.

### Iteration 602
- Yosys SVA BMC top-level composite schema operators:
  - Added schema-level operators:
    - `all_of`: every clause must hold
    - `any_of`: at least one clause must hold
  - Added clause form:
    - `keys_all`: all listed keys must be present
    - `keys_any`: at least one listed key must be present
  - Added strict schema validation for composite metadata:
    - non-empty array checks
    - unknown clause key rejection
    - duplicate key rejection in `keys_all` / `keys_any`
    - clause must define at least one of `keys_all` / `keys_any`
  - Composite checks execute in required-check phase and therefore honor
    `validate_merged_context=true` effective-context validation mode.
- Regression tests:
  - Expanded
    `test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test`
    with:
    - positive `all_of` + `any_of` merged-context route match
    - unsatisfied `all_of` rejection
    - unsatisfied `any_of` rejection
    - malformed composite clause rejection
- Validation status:
  - `bash -n utils/run_yosys_sva_circt_bmc.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test` -> 1/1 PASS
  - `build/bin/llvm-lit -sv $(rg --files test/Tools | rg 'run-yosys-sva-bmc-summary-history-drop-events.*\\.test$')` -> 16/16 PASS
  - External smoke sweep:
    ...
    - `utils/run_avip_circt_verilog.sh /home/thomas-ahle/mbit/jtag_avip`
      -> PASS
- Current limitations / debt:
  - Composite operators currently express key-presence logic only.
  - Composite clauses cannot yet encode cross-key value relationships.
  - OpenTitan LEC still needs `LEC_ACCEPT_XPROP_ONLY=1` for
    `aes_sbox_canright`.
- Long-term features to prioritize:
  - Add cross-key value predicates/operators to composite schema clauses.
  - Add parameterized/importable selector macro libraries.
...
### Iteration 604
- Yosys SVA BMC composite integer comparator operators:
  - Added clause operators:
    - `int_lt`, `int_le`, `int_gt`, `int_ge` over key-pair lists
  - Comparator checks compose with existing clause predicates:
    - `keys_all`, `keys_any`, `equals`, `not_equals`, integer comparators
  - Added strict schema diagnostics for comparator metadata:
    - malformed pair structure/type
    - duplicate ordered pairs
    - self-pairs
  - Comparator evaluation accepts typed integers and canonicalized decimal
    strings, so behavior is stable in both merged and non-merged validation.
- Regression tests:
  - Expanded
    `test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test`
    with:
    - positive merged `int_gt` clause case
...
- Validation status:
  ...
  - External smoke sweep:
    ...
    - `utils/run_avip_circt_verilog.sh /home/thomas-ahle/mbit/jtag_avip`
      -> PASS
- Current limitations / debt:
  - Comparator operators currently support key-vs-key only.
  - No key-vs-literal comparator operators yet.
  - OpenTitan LEC still needs `LEC_ACCEPT_XPROP_ONLY=1` for
    `aes_sbox_canright`.
- Long-term features to prioritize:
  - Add key-vs-literal comparator predicates in composite clauses.
  - Add parameterized/importable selector macro libraries.
...
- Yosys SVA BMC key-vs-literal integer comparators:
  - Added composite clause operators:
    - `int_lt_const`, `int_le_const`, `int_gt_const`, `int_ge_const`
      over `[key, literal]` pairs
    - `int_between` over `[key, min, max]` triples (inclusive bounds)
  - Added strict schema validation for literal/range comparators:
    - malformed pair/triple shape
    - non-integer literal rejection
    - duplicate literal pairs / duplicate ranges
    - invalid range bounds (`min > max`)
  - Added clause diagnostics/formatting support so unsatisfied `all_of` errors
    now surface literal and range predicates directly.
  - Comparator evaluation continues to accept typed integers and canonicalized
    decimal strings in merged/non-merged context validation.
- Regression tests:
  - Expanded
    `test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test`
    with:
    - positive merged schema case including `int_ge_const` + `int_between`
...
- Validation status:
  ...
  - External smoke sweep:
    ...
    - `CIRCT_VERILOG=/home/thomas-ahle/circt/build/bin/circt-verilog utils/run_avip_circt_verilog.sh /home/thomas-ahle/mbit/jtag_avip`
      -> PASS
- Current limitations / debt:
  - Literal comparators still evaluate at runtime and do not enforce declared
    integer key types at schema-parse time.
  - Composite clauses still lack arithmetic expressions (e.g. key + offset).
  - OpenTitan LEC still needs `LEC_ACCEPT_XPROP_ONLY=1` for
    `aes_sbox_canright`.
- Long-term features to prioritize:
  - Add typed-key enforcement for comparator operands during schema parsing.
  - Add arithmetic/computed comparator predicates in composite clauses.
  - Add parameterized/importable selector macro libraries.
  - Reduce OpenTitan `XPROP_ONLY` dependence via stronger 4-state and unknown
    modeling.

### Iteration 606
- Yosys SVA BMC comparator key-type enforcement at schema parse time:
  - Added schema-time validation for comparator operands used in:
    - `int_lt`, `int_le`, `int_gt`, `int_ge`
    - `int_lt_const`, `int_le_const`, `int_gt_const`, `int_ge_const`
    - `int_between`
  - Comparator operand keys must now:
    - be declared in schema `keys`
    - use `type: "integer"`
  - Invalid comparator key references now fail early when parsing
    `YOSYS_SVA_MODE_SUMMARY_HISTORY_DROP_EVENTS_REWRITE_SELECTOR_PROFILE_ROUTE_CONTEXT_SCHEMA_JSON`
    instead of failing later at route evaluation time.
- Regression tests:
  - Expanded
    `test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test`
    with:
    - undeclared comparator key rejection (`int_gt` key-pair)
...
- Validation status:
  ...
  - External smoke sweep:
    ...
    - `CIRCT_VERILOG=/home/thomas-ahle/circt/build/bin/circt-verilog utils/run_avip_circt_verilog.sh /home/thomas-ahle/mbit/jtag_avip`
      -> PASS
- Current limitations / debt:
  - Comparator clauses still cannot express arithmetic/computed constraints.
  - Comparator clauses still cannot express floating-point or string ordering.
  - OpenTitan LEC still needs `LEC_ACCEPT_XPROP_ONLY=1` for
    `aes_sbox_canright`.
- Long-term features to prioritize:
  - Add arithmetic comparator expressions (offsets/ranges computed from keys).
  - Add typed expression DSL for composite clauses (beyond direct key/literal
    pairs).
  - Add parameterized/importable selector macro libraries.
  - Reduce OpenTitan `XPROP_ONLY` dependence via stronger 4-state and unknown
    modeling.
...
### Iteration 607
- Yosys SVA BMC arithmetic comparator expressions (offset triples):
  - Added composite clause operators:
    - `int_lt_offset`, `int_le_offset`, `int_gt_offset`, `int_ge_offset`
      over `[lhs, rhs, delta]` triples interpreted as
      `lhs OP (rhs + delta)`.
  - Added strict parser validation for offset triples:
    - malformed triple shape/type
    - non-integer `delta` rejection
    - duplicate triple rejection
  - Added clause formatting for offset diagnostics, e.g.
    `int_ge_offset=[attempt>=min_attempt+2]`.
  - Extended schema-time comparator key-type enforcement so offset operators
    also require declared integer keys for both `lhs` and `rhs`.
- Regression tests:
  - Expanded
    `test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test`
    with:
    - positive merged schema case including `int_ge_offset`
...
- Validation status:
  ...
  - External smoke sweep:
    ...
    - `CIRCT_VERILOG=/home/thomas-ahle/circt/build/bin/circt-verilog utils/run_avip_circt_verilog.sh /home/thomas-ahle/mbit/jtag_avip`
      -> PASS
- Current limitations / debt:
  - Composite arithmetic predicates are limited to a single RHS key plus integer
    offset.
  - No boolean composition over arithmetic subexpressions beyond existing clause
    combinators.
  - OpenTitan LEC still needs `LEC_ACCEPT_XPROP_ONLY=1` for
    `aes_sbox_canright`.
- Long-term features to prioritize:
  - Introduce a typed expression DSL for composite clauses (multi-term
    arithmetic, nested predicates).
...
### Iteration 611
- Yosys SVA BMC boolean expression AST for route-context schema clauses:
  - Added composite clause operator:
    - `bool_expr`: list of boolean-expression AST nodes.
  - Added recursive boolean expression parser/evaluator with nodes:
    - `cmp`: `[lhs_int_expr, op, rhs_int_expr]` where
    ...
    - `not`: unary child
  - `bool_expr` integrates with existing `int_expr`/integer-expression support
    so arithmetic subexpressions can be nested under boolean combinators.
  - Added strict parser diagnostics:
    - unknown bool-expression operator rejection
    - malformed node shape rejection
    - comparator-op validation for `cmp`
    - duplicate top-level `bool_expr` clause rejection
  - Added runtime evaluation and formatted diagnostics, e.g.
    `bool_expr=[all(has(suite_dir), (attempt+1)>=(min_attempt+retry_margin+3))]`.
  - Extended schema-time typing checks:
    - all integer keys referenced in `bool_expr.cmp` integer subexpressions must
      be declared in schema keys with integer type.
- Regression tests:
  - Expanded
    `test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test`
    with:
    - positive merged schema case including `bool_expr`
    - `bool_expr` mismatch rejection
    - invalid `bool_expr` operator rejection
...
- Validation status:
  ...
  - External smoke sweep:
    ...
    - `CIRCT_VERILOG=/home/thomas-ahle/circt/build/bin/circt-verilog utils/run_avip_circt_verilog.sh /home/thomas-ahle/mbit/jtag_avip`
      -> PASS
- Current limitations / debt:
  - `bool_expr` currently supports integer comparisons and key-presence checks
    only; no first-class string/boolean value operators yet.
  - Expression semantics for integer `div`/`mod` remain implicit and
    non-versioned.
  - No expression normalization, simplification, or complexity budget controls.
  - OpenTitan LEC still needs `LEC_ACCEPT_XPROP_ONLY=1` for
    `aes_sbox_canright`.
- Long-term features to prioritize:
  - Add typed scalar expression support (string/bool constants and comparisons)
    inside `bool_expr`.
  - Add explicit, schema-versioned arithmetic semantics profile for `div/mod`.
  - Add expression canonicalization + complexity limits for deterministic,
    cheap routing.
  - Build reusable/importable selector/profile libraries over the AST DSL.
  - Reduce OpenTitan `XPROP_ONLY` dependence via stronger 4-state and unknown
    modeling.
...
### Iteration 615
- Yosys SVA BMC reusable regex definitions for schema bool expressions:
  - Added optional schema-level `regex_defs` map:
    - entry forms:
      - `"name": "pattern"`
      - `"name": {"pattern":"...", "flags":"ims"}`
  - Added bool-expression operator:
    - `regex_ref`: `[key, regex_name]`
  - `regex_ref` resolves through `regex_defs` at parse-time and reuses the
    resolved regex configuration.
- Regex engine robustness improvements:
  - Fixed `bool_expr.regex` internal AST representation to be deterministic and
    hashable (`pattern + canonical flags + flag bits`) instead of embedding
    compiled regex objects.
  - Added cached regex compilation for bool-expression evaluation.
- Validation and diagnostics:
  - schema parser now accepts top-level `regex_defs`.
  - unknown/invalid regex-ref names now fail with explicit field-qualified
    errors.
  - existing type checks still apply: regex predicates require string-typed
    keys.
- Regression tests:
  - Expanded
    `test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test`
    with:
    - positive merged schema case using `regex_defs` + `regex_ref`
    - negative unknown `regex_ref` definition rejection
  - Retained and revalidated inline regex-flags tests.
- Validation status:
  - `bash -n utils/run_yosys_sva_circt_bmc.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test` -> 1/1 PASS
  - `build/bin/llvm-lit -sv -j 1 $(rg --files test/Tools | rg 'run-yosys-sva-bmc-summary-history-drop-events.*\\.test$')` -> 16/16 PASS
...
- Current limitations / debt:
  ...
  - OpenTitan LEC still needs `LEC_ACCEPT_XPROP_ONLY=1` for
    `aes_sbox_canright`.
- Long-term features to prioritize:
  - Add selector/profile-level importable regex libraries with override rules.
  - Introduce typed scalar expression leaves and explicit scalar comparison
    semantics profiles.
  - Add schema-versioned integer arithmetic semantics and deterministic AST
    normalization.
...
### Iteration 616
- Yosys SVA BMC expression complexity budgeting for route-context schemas:
  - Added schema-level optional `limits` object:
    - `max_int_expr_nodes`
    - `max_int_expr_depth`
    - `max_bool_expr_nodes`
    - `max_bool_expr_depth`
  - Added parser-enforced budget tracking for:
    ...
    - `bool_expr` trees (including nested `all`/`any`/`not`)
    - nested integer expressions inside `bool_expr.cmp`
  - Default limits when `limits` is omitted:
    - int nodes: 256, int depth: 32
    - bool nodes: 256, bool depth: 32
- Robustness fixes:
  - Added deterministic budget accounting helpers and depth/node failure
    diagnostics at field-qualified paths.
  - Preserved existing schema/type semantics; limits are additive hardening.
- Regression tests:
  - Expanded
    `test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test`
    with:
    - bool expression depth-limit rejection
    - int expression node-limit rejection
  - Existing regex-ref/regex-flags and scalar comparator tests continue to pass.
- Validation status:
  - `bash -n utils/run_yosys_sva_circt_bmc.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test` -> 1/1 PASS
  - `build/bin/llvm-lit -sv -j 1 $(rg --files test/Tools | rg 'run-yosys-sva-bmc-summary-history-drop-events.*\\.test$')` -> 16/16 PASS
  - External smoke sweep:
    ...
    - `CIRCT_VERILOG=/home/thomas-ahle/circt/build/bin/circt-verilog LEC_ACCEPT_XPROP_ONLY=1 python3 utils/run_opentitan_circt_lec.py --opentitan-root /home/thomas-ahle/opentitan --impl-filter canright`
      -> `aes_sbox_canright` XPROP_ONLY (accepted)
- Current limitations / debt:
  - Limits are schema-local and static; there is no per-route dynamic budget
    policy.
  - Selector/profile regex definitions are still local to one schema payload;
    no shared import mechanism.
  - Scalar comparators still do not expose explicit
    collation/normalization policy for non-integer comparisons.
  - Integer arithmetic semantics (`div`/`mod`) remain non-versioned.
  - OpenTitan LEC still needs `LEC_ACCEPT_XPROP_ONLY=1` for
    `aes_sbox_canright`.
- Long-term features to prioritize:
  - Add importable selector/profile regex libraries and shared schema modules.
  - Add per-profile complexity budget profiles and observability counters.
  - Introduce typed scalar expression leaves and explicit scalar comparison
    semantics profiles.
  - Add schema-versioned integer arithmetic semantics and deterministic AST
    normalization.
...
### Iteration 617
- Yosys SVA BMC schema-versioned integer arithmetic semantics for `div`/`mod`:
  - Added optional schema-level `int_arithmetic` object:
    - `div_mode`: `floor` | `trunc_zero`
    - `mod_mode`: `floor` | `trunc_zero`
  - Default semantics preserve existing behavior:
    - `div_mode=floor`
    - `mod_mode=floor`
  - Evaluation changes:
    - `int_expr` now evaluates `div`/`mod` via configured arithmetic semantics.
    - `bool_expr.cmp` inherits the same semantics for nested int expressions.
- Robustness and diagnostics:
  - Added explicit parser validation for arithmetic mode fields with
    field-qualified errors.
  - Kept divide/mod-by-zero behavior unchanged (`None` / unsatisfied clause).
- Regression tests:
  - Expanded
    `test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test`
    with:
    - positive route-context schema case validating `trunc_zero` behavior on
      negative operands (`-3 / 2 == -1`, `-3 % 2 == -1`)
    - negative schema parse case for invalid `int_arithmetic.div_mode`
- Validation status:
  - `bash -n utils/run_yosys_sva_circt_bmc.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-yosys-sva-bmc-summary-history-drop-events-rewrite-profile-route-auto.test` -> 1/1 PASS
  - `build/bin/llvm-lit -sv -j 1 $(rg --files test/Tools | rg 'run-yosys-sva-bmc-summary-history-drop-events.*\\.test$')` -> 16/16 PASS
  - External smoke sweep:
    ...
    - `CIRCT_VERILOG=/home/thomas-ahle/circt/build/bin/circt-verilog LEC_ACCEPT_XPROP_ONLY=1 python3 utils/run_opentitan_circt_lec.py --opentitan-root /home/thomas-ahle/opentitan --impl-filter canright`
      -> `aes_sbox_canright` XPROP_ONLY (accepted)
- Current limitations / debt:
  - Arithmetic semantics are configured per schema, not per-clause/per-profile.
  - No schema-version gating yet for arithmetic mode defaults or future mode
    additions.
  ...
  - OpenTitan LEC still needs `LEC_ACCEPT_XPROP_ONLY=1` for
    `aes_sbox_canright`.
- Long-term features to prioritize:
  - Add schema-version-driven arithmetic defaulting and compatibility checks.
  - Add per-clause override hooks for arithmetic semantics where needed.
...
### Iteration 624
- sv-tests BMC smoke classification fix for simulation-negative tests:
  - Updated `utils/run_sv_tests_circt_bmc.sh` to avoid forcing `XFAIL` in
    smoke mode for `:type: simulation` tests with `:should_fail_because:`.
  - Simulation-negative tests now keep PASS/FAIL classification semantics in
    smoke mode (PASS on expected violation path) instead of unconditional XFAIL.
- Regression coverage:
  - Extended `test/Tools/run-sv-tests-bmc-simfail.test` with smoke-mode checks.
  - New assertions ensure smoke and non-smoke modes both classify the fixture as
    `PASS` (not `XFAIL`).
- Validation status:
  - `bash -n utils/run_sv_tests_circt_bmc.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-sv-tests-bmc-simfail.test` -> PASS
  - Real sv-tests confirmation:
  ...
  - Ongoing external cadence smoke (post-fix):
    - `TEST_FILTER=basic02 BMC_SMOKE_ONLY=1 utils/run_yosys_sva_circt_bmc.sh /home/thomas-ahle/yosys/tests/sva`
      -> total=2 pass=2 fail=0
    - `TEST_FILTER=basic02 BMC_SMOKE_ONLY=1 utils/run_yosys_sva_circt_lec.sh /home/thomas-ahle/yosys/tests/sva`
      -> total=1 pass=1 fail=0 error=0 skip=0
...
### Iteration 633
- Formal cadence webhook reliability and multi-sink support:
  - Updated `utils/run_formal_cadence.sh` webhook handling:
    - `--on-fail-webhook <url>` is now repeatable (multiple endpoints).
    - Added `--webhook-retries N` (per-endpoint retry count).
    - Added `--webhook-backoff-secs N` (retry delay).
    - Added `--webhook-timeout-secs N` (per-request timeout).
  - Added webhook endpoint + policy state metadata:
    - `on_fail_webhooks`, `webhook_retries`, `webhook_backoff_secs`,
      `webhook_timeout_secs`.
  - Added explicit retry-attempt logging and endpoint-level failure summary.
  - Extended webhook JSON payload with `webhook_count`.
- Regression coverage:
  - Updated `test/Tools/run-formal-cadence.test`:
    - fake curl now simulates first-attempt failure
    - verifies retry behavior (`3` calls for `2` endpoints + one retry path)
    - verifies payload marker + both endpoint URLs.
- Documentation:
  - Updated `docs/FormalRegression.md` with repeatable webhook and
    retry/backoff/timeout options.
- Validation status:
  - `bash -n utils/run_formal_cadence.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-formal-cadence.test` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-formal-all-strict-gate.test` -> PASS
- Current limitations / debt:
  - Delivery policy is still fixed-interval retry; no exponential backoff/jitter
    policy yet.
  - Notification system still lacks native SMTP/email backend.

### Iteration 634
- `run_formal_all.sh` CIRCT_VERILOG plumbing hardening:
  - Added `--circt-verilog <path>` option (default `<repo>/build/bin/circt-verilog`).
  - Added executable validation for `--with-avip` / `--with-opentitan` lanes.
  - Wired `CIRCT_VERILOG` env propagation to:
    - OpenTitan LEC invocation
    - AVIP compile lanes.
  - This removes cwd-relative `build/bin/circt-verilog` ambiguity that caused
    AVIP path failures in integrated harness runs.
- Regression coverage:
  - Updated `test/Tools/run-formal-all-strict-gate.test`:
    - adds fake AVIP lane
    - asserts `CIRCT_VERILOG` env injection via `--circt-verilog`
    - checks AVIP summary row reports PASS.
- Documentation:
  - Updated `docs/FormalRegression.md` optional-run examples with
    `--circt-verilog`.
- Validation status:
  - `bash -n utils/run_formal_all.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-formal-all-strict-gate.test` -> PASS
  - External integrated smoke:
...
- Current limitations / debt:
  - AVIP integration still depends on external VIP quality/compatibility (e.g.,
    `axi4Lite_avip` known fail in this smoke).
...
### Iteration 643
- Per-test expected failure case tracking in `utils/run_formal_all.sh`:
  - Added `--expected-failure-cases-file <file>` for case-level expectations.
  - Added `--fail-on-unexpected-failure-cases` gate.
  - Added `--fail-on-expired-expected-failure-cases` gate.
...
- Case schema and matching behavior:
  - Expected-cases TSV required columns:
    - `suite`, `mode`, `id`
  - Optional columns:
    ...
    - `reason`
  - Matching currently uses fail-like statuses parsed from suite result files
    for:
    - `sv-tests` BMC/LEC
    - `verilator-verification` BMC/LEC
    - `yosys/tests/sva` LEC
- Validation/robustness:
  - Added strict parser validation for expected-cases file:
    - required columns, duplicate rows, valid `id_kind`, valid `status`,
      valid `expires_on`.
  - Added explicit diagnostics for unexpected observed cases and expired
    expected cases.
- Regression coverage:
  - Updated `test/Tools/run-formal-all-strict-gate.test`:
    - positive case-level budget match
    - negative unexpected-case detection
    - negative expired-case detection
    - JSON summary coverage for `expected_failure_cases`.
- Documentation/schema:
  - Updated `docs/FormalRegression.md` with case-level expected-failure options
    and file format.
  - Updated `utils/formal-summary-schema.json` with optional
    `expected_failure_cases` schema.
- Validation status:
  - `bash -n utils/run_formal_all.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-formal-all-strict-gate.test` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-formal-cadence.test` -> PASS
  - Integrated smoke sweep:
    - `BMC_SMOKE_ONLY=1 LEC_SMOKE_ONLY=1 TEST_FILTER='basic02|16.9--sequence-goto-repetition|assert_fell' utils/run_formal_all.sh --out-dir /tmp/formal-results-expected-cases-smoke --with-opentitan --opentitan /home/thomas-ahle/opentitan --with-avip --avip-glob '/home/thomas-ahle/mbit/*avip*' --circt-verilog /home/thomas-ahle/circt/build/bin/circt-verilog --circt-verilog-avip /home/thomas-ahle/circt/build/bin/circt-verilog --circt-verilog-opentitan /home/thomas-ahle/circt/build/bin/circt-verilog --lec-accept-xprop-only --expected-failure-cases-file /tmp/formal-expected-cases-smoke.tsv --fail-on-unexpected-failure-cases`
      ...
      - AVIP compile lanes: PASS except `axi4Lite_avip` FAIL (known external VIP limitation)
- Current limitations / debt:
  - Case-level tracking currently does not ingest per-test outcomes from
    `yosys/tests/sva` BMC, OpenTitan LEC, or AVIP compile lanes.
  - Matching is exact only (`base` or `path`); regex/grouping semantics are not
    implemented.
  - No built-in expectation-auto-prune workflow yet for matched+expired entries.

### Iteration 644
- Aggregate expected-case support for lanes without per-test result rows:
  - Extended expected-cases matching with `id_kind=aggregate`.
  - Added synthetic aggregate fail-like observation for suite/mode rows in
    `summary.tsv` when detailed result files are unavailable.
  - Aggregate observations use:
    - `id=__aggregate__`
    - status inferred from summary counts (`fail`, `error`, `xfail`, `xpass`).
- Initial coverage impact:
  - Case-level gating can now cover `yosys/tests/sva` BMC using
    `id_kind=aggregate`.
- Regression coverage:
  - Updated `test/Tools/run-formal-all-strict-gate.test` with:
    - positive aggregate-case match scenario for `yosys/tests/sva` BMC.
- Documentation/schema:
  - Updated `docs/FormalRegression.md` to document `id_kind=aggregate`.
  - Updated `utils/formal-summary-schema.json` enum for
    `expected_failure_cases.rows[].id_kind`.
- Validation status:
  - `bash -n utils/run_formal_all.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-formal-all-strict-gate.test` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-formal-cadence.test` -> PASS
...
### Iteration 652
- Automated prune workflow for stale expected-case rows:
  - Added `--prune-expected-failure-cases-file FILE` in
    `utils/run_formal_all.sh`.
  - Added prune policies:
    - `--prune-expected-failure-cases-drop-unmatched`
    - `--prune-expected-failure-cases-drop-expired`
  - If prune is enabled and no explicit drop policy is provided, both policies
    are enabled by default (drop unmatched + expired).
...
- Implementation details:
  ...
  - Rewrites TSV in canonical schema:
    - `suite mode id id_kind status expires_on reason`
  - Emits prune diagnostics:
    - kept row count
    - dropped unmatched count
    - dropped expired count
- Regression coverage:
  - Extended `test/Tools/run-formal-all-strict-gate.test`:
    - `PRUNECASE` scenario validates:
      - matched/non-expired row is kept
      - unmatched row is dropped
      - expired row is dropped
- Documentation:
  - Updated `docs/FormalRegression.md` with prune workflow usage and policy
    defaults.
- Validation status:
  - `bash -n utils/run_formal_all.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-formal-all-strict-gate.test` -> PASS
  - `build/bin/llvm-lit -sv -j 1 $(rg --files test/Tools | rg 'run-formal-.*\\.test$')` -> 4/4 PASS
  - `build/bin/llvm-lit -sv test/Tools/run-opentitan-lec-diagnose-xprop.test test/Tools/run-opentitan-lec-x-optimistic.test test/Tools/run-opentitan-lec-no-assume-known.test` -> 3/3 PASS
  - Integrated smoke sweep with prune enabled:
    - `BMC_SMOKE_ONLY=1 LEC_SMOKE_ONLY=1 TEST_FILTER='basic02|16.9--sequence-goto-repetition|assert_fell' utils/run_formal_all.sh --out-dir /tmp/formal-results-prune-smoke --sv-tests /home/thomas-ahle/sv-tests --verilator /home/thomas-ahle/verilator-verification --yosys /home/thomas-ahle/yosys/tests/sva --with-opentitan --opentitan /home/thomas-ahle/opentitan --with-avip --avip-glob '/home/thomas-ahle/mbit/*avip*' --circt-verilog /home/thomas-ahle/circt/build/bin/circt-verilog --circt-verilog-avip /home/thomas-ahle/circt/build/bin/circt-verilog --circt-verilog-opentitan /home/thomas-ahle/circt/build/bin/circt-verilog --lec-accept-xprop-only --expected-failure-cases-file /tmp/formal-expected-cases-prune-smoke.tsv --prune-expected-failure-cases-file /tmp/formal-expected-cases-prune-smoke.tsv --prune-expected-failure-cases-drop-unmatched --prune-expected-failure-cases-drop-expired`
      ...
      - AVIP compile lanes: PASS except `axi4Lite_avip` FAIL (known)
      - prune result:
        - `kept=1 dropped_unmatched=1 dropped_expired=1`
        - `/tmp/formal-expected-cases-prune-smoke.tsv` retains only
          `avip/axi4Lite_avip compile FAIL`
- Current limitations / debt:
  - Prune currently supports expected-case files only (no prune mode yet for
...
### Iteration 654
- Unified expectation maintenance dry-run mode:
  - Added `--expectations-dry-run` to `utils/run_formal_all.sh`.
  - Applies to all expectation mutators:
    - expected-failure budget prune
    - expected-failure budget refresh
    - expected-case prune
    - expected-case refresh
  - In dry-run mode, mutators compute and report changes but do not rewrite
    target expectation files.
- Dry-run diagnostics:
  - Emits explicit messages:
  ...
  - Still emits computed row counts for planned refresh/prune outputs.
- Regression coverage:
  - Extended `test/Tools/run-formal-all-strict-gate.test`:
    - `DRYRUNBUDGET`: verifies budget file remains unchanged under dry-run
      refresh.
    - `DRYRUNCASE`: verifies case file remains unchanged under dry-run prune.
- Documentation:
  - Updated `docs/FormalRegression.md` with dry-run usage and global option
    semantics.
- Validation status:
  - `bash -n utils/run_formal_all.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-formal-all-strict-gate.test` -> PASS
  - `build/bin/llvm-lit -sv -j 1 $(rg --files test/Tools | rg 'run-formal-.*\\.test$')` -> 4/4 PASS
  - `build/bin/llvm-lit -sv test/Tools/run-opentitan-lec-diagnose-xprop.test test/Tools/run-opentitan-lec-x-optimistic.test test/Tools/run-opentitan-lec-no-assume-known.test` -> 3/3 PASS
  - Integrated smoke sweep with dry-run across all mutators:
    - `BMC_SMOKE_ONLY=1 LEC_SMOKE_ONLY=1 TEST_FILTER='basic02|16.9--sequence-goto-repetition|assert_fell' utils/run_formal_all.sh --out-dir /tmp/formal-results-dryrun-smoke --sv-tests /home/thomas-ahle/sv-tests --verilator /home/thomas-ahle/verilator-verification --yosys /home/thomas-ahle/yosys/tests/sva --with-opentitan --opentitan /home/thomas-ahle/opentitan --with-avip --avip-glob '/home/thomas-ahle/mbit/*avip*' --circt-verilog /home/thomas-ahle/circt/build/bin/circt-verilog --circt-verilog-avip /home/thomas-ahle/circt/build/bin/circt-verilog --circt-verilog-opentitan /home/thomas-ahle/circt/build/bin/circt-verilog --lec-accept-xprop-only --expected-failures-file /tmp/formal-expected-failures-dryrun-smoke.tsv --prune-expected-failures-file /tmp/formal-expected-failures-dryrun-smoke.tsv --refresh-expected-failures-file /tmp/formal-expected-failures-dryrun-smoke.tsv --expected-failure-cases-file /tmp/formal-expected-cases-dryrun-smoke.tsv --prune-expected-failure-cases-file /tmp/formal-expected-cases-dryrun-smoke.tsv --refresh-expected-failure-cases-file /tmp/formal-expected-cases-dryrun-smoke.tsv --refresh-expected-failure-cases-default-expires-on 2099-12-31 --expectations-dry-run`
      ...
      - verification:
        ...
        - printed `DRYRUN_FILES_UNCHANGED`
- Current limitations / debt:
  - No machine-readable dry-run delta artifact yet (only console summaries).
  - No per-mutator dry-run granularity; flag is global.

### Iteration 655
- Machine-readable dry-run delta reporting:
  - Added `--expectations-dry-run-report-jsonl FILE` to
    `utils/run_formal_all.sh`.
  - When used with `--expectations-dry-run`, each expectation mutator appends
    one JSON line summary to the report file.
  - Covered mutators:
    - `prune_expected_failures`
    - `refresh_expected_failures`
    - `prune_expected_failure_cases`
    - `refresh_expected_failure_cases`
- JSONL payload includes operation metadata:
  - operation name
  - target file path
  - row counts / drop counts
  - active refresh filters / collapse mode where applicable
- Regression coverage:
  - Extended `test/Tools/run-formal-all-strict-gate.test`:
    - `DRYRUNREPORT` checks report JSONL contains both refresh operation tags.
  - Existing dry-run non-mutation checks remain:
    - `DRYRUNBUDGET`
    - `DRYRUNCASE`
- Documentation:
  - Updated `docs/FormalRegression.md` with JSONL report usage and semantics.
- Validation status:
  - `bash -n utils/run_formal_all.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-formal-all-strict-gate.test` -> PASS
  - `build/bin/llvm-lit -sv -j 1 $(rg --files test/Tools | rg 'run-formal-.*\\.test$')` -> 4/4 PASS
  - `build/bin/llvm-lit -sv test/Tools/run-opentitan-lec-diagnose-xprop.test test/Tools/run-opentitan-lec-x-optimistic.test test/Tools/run-opentitan-lec-no-assume-known.test` -> 3/3 PASS
  - Integrated smoke sweep with dry-run + JSONL reporting:
    - `BMC_SMOKE_ONLY=1 LEC_SMOKE_ONLY=1 TEST_FILTER='basic02|16.9--sequence-goto-repetition|assert_fell' utils/run_formal_all.sh --out-dir /tmp/formal-results-dryrun-report-smoke --sv-tests /home/thomas-ahle/sv-tests --verilator /home/thomas-ahle/verilator-verification --yosys /home/thomas-ahle/yosys/tests/sva --with-opentitan --opentitan /home/thomas-ahle/opentitan --with-avip --avip-glob '/home/thomas-ahle/mbit/*avip*' --circt-verilog /home/thomas-ahle/circt/build/bin/circt-verilog --circt-verilog-avip /home/thomas-ahle/circt/build/bin/circt-verilog --circt-verilog-opentitan /home/thomas-ahle/circt/build/bin/circt-verilog --lec-accept-xprop-only --expected-failures-file /tmp/formal-expected-failures-dryrun-report-smoke.tsv --prune-expected-failures-file /tmp/formal-expected-failures-dryrun-report-smoke.tsv --refresh-expected-failures-file /tmp/formal-expected-failures-dryrun-report-smoke.tsv --expected-failure-cases-file /tmp/formal-expected-cases-dryrun-report-smoke.tsv --prune-expected-failure-cases-file /tmp/formal-expected-cases-dryrun-report-smoke.tsv --refresh-expected-failure-cases-file /tmp/formal-expected-cases-dryrun-report-smoke.tsv --refresh-expected-failure-cases-default-expires-on 2099-12-31 --expectations-dry-run --expectations-dry-run-report-jsonl /tmp/formal-dryrun-report-smoke.jsonl`
      ...
      - verification:
        - expectation files unchanged (`cmp` PASS)
        - report file contains 4 operations (one per mutator)
- Current limitations / debt:
...
### Iteration 658
- Dry-run JSONL explicit run completion marker:
  - Added `operation=run_end` emission in `utils/run_formal_all.sh`.
  - `run_end` is emitted from an `EXIT` trap so it is appended for both success
    and non-zero script exits.
  - `run_end` payload fields:
    - `operation`: `run_end`
    - `schema_version`: `1`
    - `date`
    - `run_id`
    - `out_dir`
    - `exit_code`
- Reporting contract after this change:
  - first row per run: `run_meta`
  - middle rows: mutator operations (`prune_*` / `refresh_*`)
  - final row per run: `run_end`
- Regression coverage:
  - Updated `test/Tools/run-formal-all-strict-gate.test`:
    - `DRYRUNREPORT` now checks `run_end` presence and `exit_code=0`.
- Documentation:
  - Updated `docs/FormalRegression.md` to document `run_end` with `exit_code`.
- Validation status:
  - `bash -n utils/run_formal_all.sh` -> PASS
  - `build/bin/llvm-lit -sv test/Tools/run-formal-all-strict-gate.test` -> PASS
  - `build/bin/llvm-lit -sv -j 1 $(rg --files test/Tools | rg 'run-formal-.*\\.test$')` -> 4/4 PASS
  ...
  - Integrated smoke verification:
    - `BMC_SMOKE_ONLY=1 LEC_SMOKE_ONLY=1 TEST_FILTER='basic02|16.9--sequence-goto-repetition|assert_fell' utils/run_formal_all.sh --out-dir /tmp/formal-results-dryrun-meta-smoke --sv-tests /home/thomas-ahle/sv-tests --verilator /home/thomas-ahle/verilator-verification --yosys /home/thomas-ahle/yosys/tests/sva --with-opentitan --opentitan /home/thomas-ahle/opentitan --with-avip --avip-glob '/home/thomas-ahle/mbit/*avip*' --circt-verilog /home/thomas-ahle/circt/build/bin/circt-verilog --circt-verilog-avip /home/thomas-ahle/circt/build/bin/circt-verilog --circt-verilog-opentitan /home/thomas-ahle/circt/build/bin/circt-verilog --lec-accept-xprop-only --expected-failures-file /tmp/formal-expected-failures-dryrun-meta-smoke.tsv --prune-expected-failures-file /tmp/formal-expected-failures-dryrun-meta-smoke.tsv --refresh-expected-failures-file /tmp/formal-expected-failures-dryrun-meta-smoke.tsv --expected-failure-cases-file /tmp/formal-expected-cases-dryrun-meta-smoke.tsv --prune-expected-failure-cases-file /tmp/formal-expected-cases-dryrun-meta-smoke.tsv --refresh-expected-failure-cases-file /tmp/formal-expected-cases-dryrun-meta-smoke.tsv --refresh-expected-failure-cases-default-expires-on 2099-12-31 --expectations-dry-run --expectations-dry-run-report-jsonl /tmp/formal-dryrun-meta-smoke.jsonl`
      - report segment contains: `run_meta`, 4 mutator rows, `run_end`
...
### Iteration 660
- Added dry-run JSONL integrity verifier tool:
  - New utility: `utils/verify_formal_dryrun_report.py`
  - Validates run segments in append-only reports:
    - starts with `run_meta`
    - ends with `run_end`
    - consistent `run_id` per segment
    - `run_end.row_count` matches observed pre-end row count
    - `run_end.payload_sha256` matches canonical row hash
    - `run_end.exit_code` type validation
- Regression coverage:
  - Updated `test/Tools/run-formal-all-strict-gate.test`:
    - positive verification call on generated dry-run report
    - negative verification call after tampering `row_count` (expects failure)
- Documentation:
  - Updated `docs/FormalRegression.md` with verifier command usage.
- Validation status:
  - `bash -n utils/run_formal_all.sh` -> PASS
  ...
  - Integrated smoke verification:
    - `BMC_SMOKE_ONLY=1 LEC_SMOKE_ONLY=1 TEST_FILTER='basic02|16.9--sequence-goto-repetition|assert_fell' utils/run_formal_all.sh --out-dir /tmp/formal-results-dryrun-meta-smoke --sv-tests /home/thomas-ahle/sv-tests --verilator /home/thomas-ahle/verilator-verification --yosys /home/thomas-ahle/yosys/tests/sva --with-opentitan --opentitan /home/thomas-ahle/opentitan --with-avip --avip-glob '/home/thomas-ahle/mbit/*avip*' --circt-verilog /home/thomas-ahle/circt/build/bin/circt-verilog --circt-verilog-avip /home/thomas-ahle/circt/build/bin/circt-verilog --circt-verilog-opentitan /home/thomas-ahle/circt/build/bin/circt-verilog --lec-accept-xprop-only --expected-failures-file /tmp/formal-expected-failures-dryrun-meta-smoke.tsv --prune-expected-failures-file /tmp/formal-expected-failures-dryrun-meta-smoke.tsv --refresh-expected-failures-file /tmp/formal-expected-failures-dryrun-meta-smoke.tsv --expected-failure-cases-file /tmp/formal-expected-cases-dryrun-meta-smoke.tsv --prune-expected-failure-cases-file /tmp/formal-expected-cases-dryrun-meta-smoke.tsv --refresh-expected-failure-cases-file /tmp/formal-expected-cases-dryrun-meta-smoke.tsv --refresh-expected-failure-cases-default-expires-on 2099-12-31 --expectations-dry-run --expectations-dry-run-report-jsonl /tmp/formal-dryrun-meta-smoke.jsonl`
      ...
      - `python3 utils/verify_formal_dryrun_report.py /tmp/formal-dryrun-meta-smoke.jsonl` -> PASS
- Current limitations / debt:
...
### Iteration 674
- Added lane sharding controls in `utils/run_formal_all.sh`:
  - `--include-lane-regex`
  - `--exclude-lane-regex`
- Established stable lane-id routing model for distributed workers.
- Planning impact:
  - Enables deterministic matrix partitioning and federated closure workflows.
  - Forms the routing substrate for checkpoint/replay and merge features.

### Iteration 675
- Added persistent lane checkpoints:
  - `--lane-state-tsv`
  - `--resume-from-lane-state`
  - `--reset-lane-state`
- Planning impact:
  - Recoverable long-running matrix runs.
  - Prerequisite for worker federation and resumable CI pipelines.

### Iteration 676
- Added lane-state compatibility fingerprinting (`config_hash`) and hash-gated
  resume behavior.
- Planning impact:
  - Prevents stale/unsafe checkpoint replay under semantic drift.
  - Establishes base for explicit compatibility-policy versioning.

### Iteration 677
- Added lane-state federation:
  - `--merge-lane-state-tsv`
  - deterministic conflict policy aligned with hash semantics.
- Planning impact:
  - Enables sharded worker runs to converge into a single resumable state.
  - Makes 24/7 distributed closure practical without re-running completed lanes.

### Iteration 678
- Added lane-state inspection/audit utility `utils/inspect_formal_lane_state.py`.
- Planning impact:
