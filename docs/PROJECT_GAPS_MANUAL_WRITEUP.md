...
### 1400. `lib/Conversion/ImportVerilog/ImportVerilog.cpp:1838`
Pipeline TODO: `SimplifyProcedures` is disabled due interference with `@(...)` event-control checks. What is missing is a fix that preserves correct event-observation semantics so pass can be re-enabled.
