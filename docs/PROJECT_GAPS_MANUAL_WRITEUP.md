...
### [x] 2091. `test/Tools/crun/uvm-factory-create-null-parent.sv:1`
Status update (2026-03-01): this gap is closed in this workspace. Removed `XFAIL` and made the test semantically legal by creating the null-parent component in `build_phase` (run-phase component creation is illegal in UVM). Root cause of prior null return was wrapper-factory interception treating null create results as success and short-circuiting fallback; fixed by treating null wrapper-create results as interceptor misses so component-create fallback/canonical execution proceeds (`tools/circt-sim/LLHDProcessInterpreter.cpp`). The null-parent create semantic check now passes under crun.
...
Status update (2026-03-01): this gap is closed in this workspace. Removed stale `XFAIL` and fixed the semantic runtime blocker in multi-agent sequencing: both drivers could wake from unresolved `get_next_item` waits and immediately re-block because queue routing remained ambiguous under multi-queue push. Runtime hardening now preserves queue wake-affinity per waiting process and reuses that hint on retry when structural resolution still misses (`tools/circt-sim/LLHDProcessInterpreter.h`, `tools/circt-sim/LLHDProcessInterpreter.cpp`, `tools/circt-sim/LLHDProcessInterpreterCallIndirect.cpp`). Multi-agent parallel sequencing now passes consistently (`agent_a produced 4 items`, `agent_b produced 3 items`, `multi-agent: PASS`).
