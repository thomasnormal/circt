...
2. WS1: in progress
   ...
   - pairwise BMC final JSONL projection now uses shared
     `formal_results.write_results_jsonl_from_case_rows` plumbing with
   ...
   - pairwise shared-path invalid `BMC_LOG_MAX_BYTES` diagnostics are now
     regression-covered
     (`run-pairwise-circt-bmc-log-max-bytes-invalid.test`)
