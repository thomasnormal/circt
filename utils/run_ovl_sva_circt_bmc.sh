  ...
  for raw_profile in "${profiles[@]}"; do
    ...
    if run_limited "$CIRCT_BMC" "${bmc_args[@]}" "$mlir" > /dev/null 2>"$bmc_log"; then
      ...
      if [[ "$BMC_SMOKE_ONLY" != "1" ]] && \
        ...
        echo "SMT-LIB export failed($base/$profile): no native fallback available after JIT removal" >&2
        {
          echo "[run_ovl_sva_circt_bmc] SMT-LIB export failed($base/$profile): no native fallback available after JIT removal"
        } >> "$bmc_log"
      fi
      failures=$((failures + 1))
      if [[ -n "$OUT" ]]; then
