...
Options:
  ...
  --fail-on-new-lec-circt-opt-error-case-reasons
                         Fail when new LEC `CIRCT_OPT_ERROR` case+reason
                         tuples (`case_id::reason`) appear vs baseline
  --fail-on-new-lec-circt-verilog-error-reason-keys
                         Fail when new LEC `CIRCT_VERILOG_ERROR` reason
                         counter keys
                         (`lec_circt_verilog_error_reason_*_cases`) appear vs
                         baseline
  --fail-on-new-lec-circt-lec-error-reason-keys
                         Fail when new LEC `CIRCT_LEC_ERROR` reason counter
                         keys (`lec_circt_lec_error_reason_*_cases`) appear vs
                         baseline
  --fail-on-new-lec-circt-lec-error-case-ids
                         Fail when LEC `CIRCT_LEC_ERROR` case IDs increase
                         vs baseline
  ...
  --fail-on-new-lec-runner-command-case-ids
                         Fail when new LEC runner-command case IDs appear vs
                         baseline
  ...
  --fail-on-new-lec-semantic-diag-subfamily-case-reasons
                         Fail when new LEC semantic-diag subfamily case+reason
                         tuples (`lec_semantic_diag_subfamily_case_reasons`)
                         appear vs baseline
  ...
  --fail-on-bmc-lec-contract-fingerprint-case-id-parity
                         Fail when BMC contract fingerprint case IDs
                         (`case_id::fingerprint`) are not present in
                         current LEC contract fingerprint case IDs
                         for the same suite
  ...
  --fail-on-bmc-lec-contract-fingerprint-case-id-map-unmapped
                         ...
                         LEC case IDs for the same suite
  --fail-on-new-bmc-lec-contract-fingerprint-case-id-map-unmapped
                         Fail when new BMC case IDs using identity fallback
                         in the case-ID map are missing in current LEC
                         case IDs vs baseline for the same suite
  ...
  --fail-on-new-mutation-lec-contract-fingerprint-lane-map-identity-fallback
                         Fail when new mutation lanes (vs baseline window)
                         fall back to identity map despite a lane-map file
                         being provided
...
FAIL_ON_NEW_LEC_SEMANTIC_DIAG_SUBFAMILY_CASE_REASONS=0
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --fail-on-new-lec-circt-lec-error-reason-keys)
      FAIL_ON_NEW_LEC_CIRCT_LEC_ERROR_REASON_KEYS=1; shift ;;
    --fail-on-new-lec-circt-lec-error-case-ids)
    ...
    --fail-on-new-lec-semantic-diag-subfamily-case-reasons)
      FAIL_ON_NEW_LEC_SEMANTIC_DIAG_SUBFAMILY_CASE_REASONS=1; shift ;;
    --fail-on-new-lec-circt-verilog-error-case-ids)
    ...
    --fail-on-new-bmc-lec-contract-fingerprint-case-id-map-unmapped)
      FAIL_ON_NEW_BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_MAP_UNMAPPED=1; shift ;;
    --fail-on-new-mutation-contract-fingerprint-case-ids)
    ...
    --fail-on-new-mutation-lec-contract-fingerprint-lane-parity)
      FAIL_ON_NEW_MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_PARITY=1; shift ;;
    --fail-on-mutation-lec-contract-fingerprint-lane-map-unmapped)
...
if [[ "$FAIL_ON_BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_MAP_UNMAPPED" == "1" && -z "$BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_MAP_FILE" ]]; then
  echo "--fail-on-bmc-lec-contract-fingerprint-case-id-map-unmapped requires --bmc-lec-contract-fingerprint-case-id-map-file" >&2
  exit 1
fi
if [[ "$FAIL_ON_NEW_BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_MAP_UNMAPPED" == "1" && -z "$BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_MAP_FILE" ]]; then
  echo "--fail-on-new-bmc-lec-contract-fingerprint-case-id-map-unmapped requires --bmc-lec-contract-fingerprint-case-id-map-file" >&2
  exit 1
fi
...
if [[ "$FAIL_ON_NEW_MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_MAP_IDENTITY_FALLBACK" == "1" && -z "$MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_MAP_FILE" ]]; then
...
if [[ -n "$MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_MAP_FILE" && "$FAIL_ON_MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_PARITY" != "1" && "$FAIL_ON_NEW_MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_PARITY" != "1" && "$STRICT_GATE" != "1" && "$FAIL_ON_MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_MAP_UNMAPPED" != "1" && "$FAIL_ON_NEW_MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_MAP_UNMAPPED" != "1" && "$FAIL_ON_MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_MAP_IDENTITY_FALLBACK" != "1" && "$FAIL_ON_NEW_MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_MAP_IDENTITY_FALLBACK" != "1" ]]; then
  echo "--mutation-lec-contract-fingerprint-lane-map-file requires --fail-on-mutation-lec-contract-fingerprint-lane-parity, --fail-on-new-mutation-lec-contract-fingerprint-lane-parity, --fail-on-mutation-lec-contract-fingerprint-lane-map-unmapped, --fail-on-new-mutation-lec-contract-fingerprint-lane-map-unmapped, --fail-on-mutation-lec-contract-fingerprint-lane-map-identity-fallback, --fail-on-new-mutation-lec-contract-fingerprint-lane-map-identity-fallback, or --strict-gate" >&2
  exit 1
fi
...
if [[ "$STRICT_GATE" == "1" ]]; then
  ...
  FAIL_ON_NEW_LEC_SEMANTIC_DIAG_SUBFAMILY_CASE_REASONS=1
  ...
  if [[ -n "$MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_MAP_FILE" ]]; then
    FAIL_ON_NEW_MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_PARITY=1
  ...
  if [[ -n "$BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_MAP_FILE" ]]; then
    FAIL_ON_NEW_BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_MAP_UNMAPPED=1
  fi
...
for xprop_prefix in "${FAIL_ON_NEW_OPENTITAN_LEC_STRICT_XPROP_COUNTER_PREFIXES[@]}"; do
  if [[ ! "$xprop_prefix" =~ ^[a-z][a-z0-9_]*$ ]]; then
    echo "invalid --fail-on-new-opentitan-lec-strict-xprop-counter-prefix: expected [a-z][a-z0-9_]*, got '$xprop_prefix'" >&2
    exit 1
  fi
done
...
def classify_error_bucket(diag: str, reason_key: str) -> str:
    if diag in {"circt_opt_error", "circt_verilog_error"}:
    ...
    for line in f:
        ...
        if re.fullmatch(r"[A-Z0-9_]+", maybe_diag):
            ...
            if status == "error" and diag == "circt_verilog_error":
                ...
                if case_id:
                    error_bucket_case_ids.add(f"{error_bucket}::{case_id}")
            if status == "error" and diag == "circt_lec_error":
                reason_key = normalize(reason_token) if reason_token else "missing"
                counts[f"lec_circt_lec_error_reason_{reason_key}_cases"] += 1
                if case_id:
...
counts["lec_circt_lec_error_case_ids_cardinality"] = len(circt_lec_error_case_ids)
counts["lec_circt_lec_error_case_reasons_cardinality"] = len(circt_lec_error_case_reasons)
...
summarize_bmc_case_file() {
  ...
  local frontend_error_reason_file="${3:-}"
  if [[ ! -s "$case_file" ]]; then
    echo ""
    return 0
  fi
...
PY
  ...
  local frontend_error_summary=""
  if [[ -n "$frontend_error_reason_file" && -s "$frontend_error_reason_file" ]]; then
    frontend_error_summary="$(
      BMC_FRONTEND_ERROR_REASON_FILE="$frontend_error_reason_file" \
      python3 - <<'PY'
import os
import re
from collections import defaultdict
...
path = Path(os.environ["BMC_FRONTEND_ERROR_REASON_FILE"])
if not path.exists():
    print("")
    raise SystemExit(0)
...
with path.open(encoding="utf-8") as f:
    for line in f:
        ...
        if not reason:
            continue
        case_id = base if base else file_path
        if not case_id:
            continue
        case_ids.add(case_id)
        case_reason_ids.add(f"{case_id}::{reason}")
        reason_key = normalize(reason)
        reason_counts[f"bmc_frontend_error_reason_{reason_key}_cases"] += 1

summary_parts = [
    f"bmc_frontend_error_cases={len(case_ids)}",
    f"bmc_frontend_error_case_reasons_cardinality={len(case_reason_ids)}",
]
summary_parts.extend(
    f"{key}={reason_counts[key]}" for key in sorted(reason_counts.keys())
)
...
if [[ -d "$SV_TESTS_DIR" ]] && lane_enabled "sv-tests/BMC"; then
  if lane_resume_from_state "sv-tests/BMC"; then
    ...
    if [[ -n "$line" ]]; then
      ...
      bmc_case_summary="$(summarize_bmc_case_file "$OUT_DIR/sv-tests-bmc-results.txt" "$sv_bmc_timeout_reasons_file" "$sv_bmc_frontend_error_reasons_file")"
...
if [[ "$WITH_SV_TESTS_UVM_BMC_SEMANTICS" == "1" ]] && \
  ...
  if lane_resume_from_state "sv-tests-uvm/BMC_SEMANTICS"; then
    ...
    if [[ -n "$line" ]]; then
      ...
      bmc_case_summary="$(summarize_bmc_case_file "$sv_bmc_uvm_semantics_results_file" "$sv_bmc_uvm_semantics_timeout_reasons_file" "$sv_bmc_uvm_semantics_frontend_error_reasons_file")"
      if [[ -n "$bmc_case_summary" ]]; then
        summary="${summary} ${bmc_case_summary}"
      fi
...
if [[ -d "$VERILATOR_DIR" ]] && lane_enabled "verilator-verification/BMC"; then
  if lane_resume_from_state "verilator-verification/BMC"; then
    ...
    if [[ -n "$line" ]]; then
      ...
      bmc_case_summary="$(summarize_bmc_case_file "$OUT_DIR/verilator-bmc-results.txt" "$verilator_bmc_timeout_reasons_file" "$verilator_bmc_frontend_error_reasons_file")"
      if [[ -n "$bmc_case_summary" ]]; then
        summary="${summary} ${bmc_case_summary}"
      fi
...
if [[ -d "$YOSYS_DIR" ]] && lane_enabled "yosys/tests/sva/BMC"; then
  if lane_resume_from_state "yosys/tests/sva/BMC"; then
    ...
    if [[ -n "$line" ]]; then
      ...
      bmc_case_summary="$(summarize_bmc_case_file "$OUT_DIR/yosys-bmc-results.txt" "$yosys_bmc_timeout_reasons_file" "$yosys_bmc_frontend_error_reasons_file")"
      if [[ -n "$bmc_case_summary" ]]; then
        summary="${summary} ${bmc_case_summary}"
      fi
...
run_opentitan_bmc_lane() {
  ...
  local frontend_error_reasons_file="$9"
  local resolved_contracts_file="${10}"
  local lane_assume_known_inputs="${11}"
  ...
  local opentitan_bmc_env=(OUT="$case_results"
    ...
    BMC_FRONTEND_ERROR_REASON_CASES_OUT="$frontend_error_reasons_file"
  ...
  if [[ ! -s "$case_results" ]]; then
    ...
    if [[ -f "$suite_log" ]] && \
      ...
      bmc_case_summary="$(summarize_bmc_case_file "$case_results" "$timeout_reasons_file" "$frontend_error_reasons_file")"
      if [[ -n "$bmc_case_summary" ]]; then
...
if [[ "$WITH_OPENTITAN_BMC_STRICT" == "1" ]]; then
  run_opentitan_bmc_lane \
    ...
    "$OUT_DIR/opentitan-bmc-strict-frontend-error-reasons.tsv" \
...
def collect_bmc_frontend_error_case_reasons(out_dir: Path):
    sources = [
        ("sv-tests", "BMC", out_dir / "sv-tests-bmc-frontend-error-reasons.tsv"),
        (
            "sv-tests-uvm",
            "BMC_SEMANTICS",
            out_dir / "sv-tests-bmc-uvm-semantics-frontend-error-reasons.tsv",
        ),
        (
            "verilator-verification",
            "BMC",
            out_dir / "verilator-bmc-frontend-error-reasons.tsv",
        ),
        ("yosys/tests/sva", "BMC", out_dir / "yosys-bmc-frontend-error-reasons.tsv"),
        ("opentitan", "BMC", out_dir / "opentitan-bmc-frontend-error-reasons.tsv"),
        (
            "opentitan",
            "BMC_STRICT",
            out_dir / "opentitan-bmc-strict-frontend-error-reasons.tsv",
...
def collect_lec_circt_opt_error_case_reasons(out_dir: Path):
    sources = [
        ("sv-tests", "LEC", out_dir / "sv-tests-lec-results.txt"),
        ("verilator-verification", "LEC", out_dir / "verilator-lec-results.txt"),
        ("yosys/tests/sva", "LEC", out_dir / "yosys-lec-results.txt"),
    ...
    for suite, mode, path in sources:
        ...
        with path.open() as f:
            for line in f:
                ...
                reason = parts[6].strip() if len(parts) > 6 else ""
                diag = extract_diag_tag(file_path, explicit_diag)
                if diag != "CIRCT_OPT_ERROR" or not reason:
                    continue
                case_id = compose_case_id(base, file_path, explicit_diag)
                if case_id:
                    case_reason_ids.setdefault(key, set()).add(f"{case_id}::{reason}")
...
def collect_lec_circt_lec_error_case_ids(out_dir: Path):
    sources = [
        ("sv-tests", "LEC", out_dir / "sv-tests-lec-results.txt"),
        ("verilator-verification", "LEC", out_dir / "verilator-lec-results.txt"),
        ("yosys/tests/sva", "LEC", out_dir / "yosys-lec-results.txt"),
...
def collect_lec_circt_lec_error_case_reasons(out_dir: Path):
    sources = [
        ("sv-tests", "LEC", out_dir / "sv-tests-lec-results.txt"),
        ("verilator-verification", "LEC", out_dir / "verilator-lec-results.txt"),
        ("yosys/tests/sva", "LEC", out_dir / "yosys-lec-results.txt"),
    ...
    for suite, mode, path in sources:
        ...
        with path.open() as f:
            for line in f:
                ...
                reason = parts[6].strip() if len(parts) > 6 else ""
                diag = extract_diag_tag(file_path, explicit_diag)
                if diag != "CIRCT_LEC_ERROR" or not reason:
                    continue
                case_id = compose_case_id(base, file_path, explicit_diag)
                if case_id:
                    case_reason_ids.setdefault(key, set()).add(f"{case_id}::{reason}")
        ...
        for key, raw_values in source.items():
            ...
            for token in raw_values.split(";"):
                ...
                if not token or "::" not in token:
                    continue
                _, reason = token.rsplit("::", 1)
                if not reason.startswith("runner_command_"):
                    continue
                case_reason_ids.setdefault(key, set()).add(token)
    return {key: ";".join(sorted(values)) for key, values in case_reason_ids.items()}

def collect_lec_runner_command_case_ids(lec_runner_command_case_reasons: dict):
    case_ids = {}
    for key, raw_values in lec_runner_command_case_reasons.items():
        if not raw_values:
            continue
        for token in raw_values.split(";"):
            token = token.strip()
            if not token or "::" not in token:
                continue
            case_id, _ = token.rsplit("::", 1)
            case_id = case_id.strip()
            if case_id:
                case_ids.setdefault(key, set()).add(case_id)
    return {key: ";".join(sorted(values)) for key, values in case_ids.items()}

def classify_lec_error_bucket(diag: str, reason: str) -> str:
    if diag in {"CIRCT_OPT_ERROR", "CIRCT_VERILOG_ERROR"}:
...
def collect_lec_semantic_diag_subfamily_case_reasons(out_dir: Path):
    sources = [
        ("sv-tests", "LEC", out_dir / "sv-tests-lec-results.txt"),
        ("verilator-verification", "LEC", out_dir / "verilator-lec-results.txt"),
        ("yosys/tests/sva", "LEC", out_dir / "yosys-lec-results.txt"),
    ...
    for suite, mode, path in sources:
        ...
        with path.open() as f:
            for line in f:
                ...
                if not case_id:
                    continue
                reason_key = re.sub(r"[^a-z0-9]+", "_", (reason or "").lower()).strip("_")
                if not reason_key:
                    reason_key = "missing"
                subfamily_case_reasons.setdefault(key, set()).add(
                    f"{subfamily}::{case_id}::{reason_key}"
                )
    return {key: ";".join(sorted(values)) for key, values in subfamily_case_reasons.items()}
...
bmc_frontend_error_case_reason_ids = collect_bmc_frontend_error_case_reasons(out_dir)
...
lec_semantic_diag_subfamily_case_reasons = (
    collect_lec_semantic_diag_subfamily_case_reasons(out_dir)
)
    ...
    with baseline_path.open() as f:
        ...
        for row in reader:
            ...
            baseline[key] = {
                ...
                'bmc_frontend_error_case_reason_ids': row.get('bmc_frontend_error_case_reason_ids', ''),
                ...
                'lec_circt_lec_error_case_ids': row.get('lec_circt_lec_error_case_ids', ''),
                'lec_circt_lec_error_case_reasons': row.get('lec_circt_lec_error_case_reasons', ''),
                ...
                'lec_semantic_diag_subfamily_case_reasons': row.get('lec_semantic_diag_subfamily_case_reasons', ''),
            }
...
for row in rows:
    ...
    baseline[key] = {
        ...
        'bmc_frontend_error_case_reason_ids': bmc_frontend_error_case_reason_ids.get((row['suite'], row['mode']), ''),
        ...
        'lec_circt_lec_error_case_ids': lec_circt_lec_error_case_ids.get((row['suite'], row['mode']), ''),
        'lec_circt_lec_error_case_reasons': lec_circt_lec_error_case_reasons.get((row['suite'], row['mode']), ''),
...
with baseline_path.open('w', newline='') as f:
    writer = csv.DictWriter(
        ...
        fieldnames=[
            ...
            'bmc_frontend_error_case_reason_ids',
...
if [[ "$FAIL_ON_NEW_XPASS" == "1" || \
      ...
      "$FAIL_ON_NEW_BMC_LEC_CONTRACT_FINGERPRINT_PARITY" == "1" || \
      "$FAIL_ON_BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_PARITY" == "1" || \
      "$FAIL_ON_NEW_BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_PARITY" == "1" || \
      "$FAIL_ON_BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_MAP_UNMAPPED" == "1" || \
      "$FAIL_ON_NEW_BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_MAP_UNMAPPED" == "1" || \
      "$FAIL_ON_NEW_MUTATION_CONTRACT_FINGERPRINT_CASE_IDS" == "1" || \
      "$FAIL_ON_NEW_MUTATION_SOURCE_FINGERPRINT_CASE_IDS" == "1" || \
      "$FAIL_ON_NEW_MUTATION_PROVENANCE_TUPLE_IDS" == "1" || \
  ...
  FAIL_ON_NEW_LEC_SEMANTIC_DIAG_SUBFAMILY_CASE_REASONS="$FAIL_ON_NEW_LEC_SEMANTIC_DIAG_SUBFAMILY_CASE_REASONS" \
...
def classify_gate_rule_id(suite: str, mode: str, detail: str):
    ...
    if detail.startswith("BMC/LEC contract fingerprint case-ID parity mismatch"):
        return "strict_gate.bmc.parity.contract_fingerprint_case_ids.missing_in_lec"
    if detail.startswith("new BMC/LEC contract fingerprint case-ID parity mismatch observed"):
        return "strict_gate.bmc.parity.contract_fingerprint_case_ids.new_missing_in_lec"
    if detail.startswith("BMC/LEC case-ID map identity fallback missing in LEC"):
        return "strict_gate.bmc.parity.contract_fingerprint_case_id_map_unmapped.missing_in_lec"
    if detail.startswith("new BMC/LEC case-ID map identity fallback missing in LEC observed"):
        return "strict_gate.bmc.parity.contract_fingerprint_case_id_map_unmapped.new_missing_in_lec"
...
def map_mutation_to_lec_lane_id_with_source(mutation_lane: str):
    ...
    for mutation_prefix, lec_prefix, _ in mutation_lec_contract_fingerprint_lane_prefix_rules:
        if mutation_lane.startswith(mutation_prefix):
            return f"{lec_prefix}{mutation_lane[len(mutation_prefix):]}", "prefix"
...
def extract_fingerprint_token(identity_token: str) -> str:
    token = (identity_token or "").strip()
    if not token or "::" not in token:
        return ""
...
current_bmc_frontend_error_case_reasons = collect_bmc_frontend_error_case_reasons(
    Path(os.environ["OUT_DIR"])
)
...
current_lec_circt_lec_error_case_ids = (
    collect_lec_circt_lec_error_case_ids(Path(os.environ["OUT_DIR"]))
)
current_lec_circt_lec_error_case_reasons = (
    collect_lec_circt_lec_error_case_reasons(Path(os.environ["OUT_DIR"]))
)
    ...
    for source in (
        ...
        for key, reason_tokens in source.items():
            for token in reason_tokens:
                ...
                _, reason = token.rsplit("::", 1)
                if reason.startswith("runner_command_"):
                    case_reasons.setdefault(key, set()).add(token)
    return case_reasons

def collect_lec_runner_command_case_ids(lec_runner_command_case_reasons: dict):
    case_ids = {}
    for key, reason_tokens in lec_runner_command_case_reasons.items():
        for token in reason_tokens:
            if "::" not in token:
                continue
            case_id, _ = token.rsplit("::", 1)
...
current_lec_semantic_diag_subfamily_case_reasons = (
    collect_lec_semantic_diag_subfamily_case_reasons(Path(os.environ["OUT_DIR"]))
)
...
fail_on_new_lec_circt_opt_error_case_reasons = (
    os.environ.get("FAIL_ON_NEW_LEC_CIRCT_OPT_ERROR_CASE_REASONS", "0") == "1"
)
fail_on_new_lec_circt_verilog_error_reason_keys = (
    os.environ.get("FAIL_ON_NEW_LEC_CIRCT_VERILOG_ERROR_REASON_KEYS", "0") == "1"
)
fail_on_new_lec_circt_lec_error_reason_keys = (
    os.environ.get("FAIL_ON_NEW_LEC_CIRCT_LEC_ERROR_REASON_KEYS", "0") == "1"
)
fail_on_new_lec_runner_command_reason_keys = (
    os.environ.get("FAIL_ON_NEW_LEC_RUNNER_COMMAND_REASON_KEYS", "0") == "1"
)
fail_on_new_lec_runner_command_case_ids = (
    os.environ.get("FAIL_ON_NEW_LEC_RUNNER_COMMAND_CASE_IDS", "0") == "1"
)
...
fail_on_new_lec_semantic_diag_subfamily_case_reasons = (
    os.environ.get("FAIL_ON_NEW_LEC_SEMANTIC_DIAG_SUBFAMILY_CASE_REASONS", "0") == "1"
)
...
fail_on_new_lec_circt_lec_error_case_ids = (
    os.environ.get("FAIL_ON_NEW_LEC_CIRCT_LEC_ERROR_CASE_IDS", "0") == "1"
)
...
fail_on_bmc_lec_contract_fingerprint_case_id_parity = (
    os.environ.get("FAIL_ON_BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_PARITY", "0")
    == "1"
)
fail_on_new_bmc_lec_contract_fingerprint_case_id_parity = (
    os.environ.get("FAIL_ON_NEW_BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_PARITY", "0")
    == "1"
)
fail_on_bmc_lec_contract_fingerprint_case_id_map_unmapped = (
    os.environ.get("FAIL_ON_BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_MAP_UNMAPPED", "0")
    == "1"
)
fail_on_new_bmc_lec_contract_fingerprint_case_id_map_unmapped = (
    os.environ.get(
        "FAIL_ON_NEW_BMC_LEC_CONTRACT_FINGERPRINT_CASE_ID_MAP_UNMAPPED", "0"
    )
    == "1"
...
fail_on_new_mutation_lec_contract_fingerprint_lane_parity = (
    os.environ.get(
        "FAIL_ON_NEW_MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_PARITY", "0"
    )
    == "1"
...
fail_on_new_mutation_lec_contract_fingerprint_lane_map_identity_fallback = (
    os.environ.get(
        "FAIL_ON_NEW_MUTATION_LEC_CONTRACT_FINGERPRINT_LANE_MAP_IDENTITY_FALLBACK",
        "0",
    )
...
fail_on_new_bmc_frontend_error_case_reasons = (
    os.environ.get("FAIL_ON_NEW_BMC_FRONTEND_ERROR_CASE_REASONS", "0") == "1"
)
...
for key, current_row in summary.items():
    ...
    if fail_on_new_bmc_frontend_error_case_reasons and mode.startswith("BMC"):
        baseline_frontend_error_case_reason_ids_raw = [
            row.get("bmc_frontend_error_case_reason_ids") for row in compare_rows
        ]
        if any(raw is not None for raw in baseline_frontend_error_case_reason_ids_raw):
            baseline_frontend_error_case_reason_set = set()
            for raw in baseline_frontend_error_case_reason_ids_raw:
                if raw is None or raw == "":
                    continue
                for token in raw.split(";"):
                    token = token.strip()
                    if token:
                        baseline_frontend_error_case_reason_set.add(token)
            current_frontend_error_case_reason_set = (
                current_bmc_frontend_error_case_reasons.get(key, set())
            )
            new_frontend_error_case_reasons = sorted(
                current_frontend_error_case_reason_set
                - baseline_frontend_error_case_reason_set
            )
            if new_frontend_error_case_reasons:
                sample = ", ".join(new_frontend_error_case_reasons[:2])
                if len(new_frontend_error_case_reasons) > 2:
                    sample += ", ..."
                gate_errors.add(
                    suite,
                    mode,
                    (
                        "new BMC frontend-error case-reason tuples observed "
                        f"(baseline={len(baseline_frontend_error_case_reason_set)} "
                        f"current={len(current_frontend_error_case_reason_set)}, "
                        f"window={baseline_window}): {sample}"
                    ),
                    rule_id="strict_gate.bmc.frontend_error.case_reasons.new",
                )
    ...
    if fail_on_new_lec_circt_opt_error_case_ids and mode.startswith("LEC"):
        ...
        if any(raw is not None for raw in baseline_opt_error_case_ids_raw):
            baseline_opt_error_case_set = set()
            for raw in baseline_opt_error_case_ids_raw:
                if raw is None or raw == "":
                    continue
                for token in raw.split(";"):
                    token = token.strip()
                    if token:
                        baseline_opt_error_case_set.add(token)
            current_opt_error_case_set = current_lec_circt_opt_error_case_ids.get(
                key, set()
            )
            new_opt_error_cases = sorted(
                current_opt_error_case_set - baseline_opt_error_case_set
            )
            if new_opt_error_cases:
                sample = ", ".join(new_opt_error_cases[:3])
                if len(new_opt_error_cases) > 3:
                    sample += ", ..."
                gate_errors.append(
                    f"{suite} {mode}: new LEC CIRCT_OPT_ERROR case IDs observed (baseline={len(baseline_opt_error_case_set)} current={len(current_opt_error_case_set)}, window={baseline_window}): {sample}"
                )
    if fail_on_new_lec_circt_opt_error_case_reasons and mode.startswith("LEC"):
        baseline_opt_error_case_reasons_raw = [
            row.get("lec_circt_opt_error_case_reasons") for row in compare_rows
        ]
        if any(raw is not None for raw in baseline_opt_error_case_reasons_raw):
            baseline_opt_error_case_reasons = set()
            for raw in baseline_opt_error_case_reasons_raw:
                if raw is None or raw == "":
                    continue
                for token in raw.split(";"):
                    token = token.strip()
                    if token:
                        baseline_opt_error_case_reasons.add(token)
            current_opt_error_case_reasons = current_lec_circt_opt_error_case_reasons.get(
                key, set()
            )
            new_opt_error_case_reasons = sorted(
                current_opt_error_case_reasons - baseline_opt_error_case_reasons
            )
            if new_opt_error_case_reasons:
                sample = ", ".join(new_opt_error_case_reasons[:2])
                if len(new_opt_error_case_reasons) > 2:
                    sample += ", ..."
                gate_errors.append(
    ...
    if fail_on_new_lec_circt_lec_error_case_ids and mode.startswith("LEC"):
        baseline_lec_error_case_ids_raw = [
            row.get("lec_circt_lec_error_case_ids") for row in compare_rows
        ]
        if any(raw is not None for raw in baseline_lec_error_case_ids_raw):
            baseline_lec_error_case_set = set()
            for raw in baseline_lec_error_case_ids_raw:
                if raw is None or raw == "":
                    continue
                for token in raw.split(";"):
                    token = token.strip()
                    if token:
                        baseline_lec_error_case_set.add(token)
            current_lec_error_case_set = (
                current_lec_circt_lec_error_case_ids.get(key, set())
            )
            new_lec_error_cases = sorted(
                current_lec_error_case_set - baseline_lec_error_case_set
            )
            if new_lec_error_cases:
                sample = ", ".join(new_lec_error_cases[:3])
                if len(new_lec_error_cases) > 3:
                    sample += ", ..."
                gate_errors.append(
                    f"{suite} {mode}: new LEC CIRCT_LEC_ERROR case IDs observed (baseline={len(baseline_lec_error_case_set)} current={len(current_lec_error_case_set)}, window={baseline_window}): {sample}"
                )
    if fail_on_new_lec_circt_lec_error_case_reasons and mode.startswith("LEC"):
        baseline_lec_error_case_reasons_raw = [
            row.get("lec_circt_lec_error_case_reasons") for row in compare_rows
        ]
        if any(raw is not None for raw in baseline_lec_error_case_reasons_raw):
            baseline_lec_error_case_reasons = set()
            for raw in baseline_lec_error_case_reasons_raw:
                if raw is None or raw == "":
                    continue
                for token in raw.split(";"):
                    token = token.strip()
                    if token:
                        baseline_lec_error_case_reasons.add(token)
            current_lec_error_case_reasons = (
                current_lec_circt_lec_error_case_reasons.get(key, set())
            )
            new_lec_error_case_reasons = sorted(
                current_lec_error_case_reasons - baseline_lec_error_case_reasons
            )
            if new_lec_error_case_reasons:
                sample = ", ".join(new_lec_error_case_reasons[:2])
                if len(new_lec_error_case_reasons) > 2:
                    sample += ", ..."
                gate_errors.append(
                    f"{suite} {mode}: new LEC CIRCT_LEC_ERROR case+reason tuples observed (baseline={len(baseline_lec_error_case_reasons)} current={len(current_lec_error_case_reasons)}, window={baseline_window}): {sample}"
                )
    if fail_on_new_lec_runner_command_case_ids and mode.startswith("LEC"):
        baseline_runner_command_case_ids_raw = [
            row.get("lec_runner_command_case_ids") for row in compare_rows
        ]
        if any(raw is not None for raw in baseline_runner_command_case_ids_raw):
            baseline_runner_command_case_ids = set()
            for raw in baseline_runner_command_case_ids_raw:
                if raw is None or raw == "":
                    continue
                for token in raw.split(";"):
                    token = token.strip()
                    if token:
                        baseline_runner_command_case_ids.add(token)
            current_runner_command_case_ids = current_lec_runner_command_case_ids.get(
                key, set()
            )
            new_runner_command_case_ids = sorted(
                current_runner_command_case_ids - baseline_runner_command_case_ids
            )
            if new_runner_command_case_ids:
                sample = ", ".join(new_runner_command_case_ids[:3])
                if len(new_runner_command_case_ids) > 3:
                    sample += ", ..."
                gate_errors.append(
                    f"{suite} {mode}: new LEC runner-command case IDs observed (baseline={len(baseline_runner_command_case_ids)} current={len(current_runner_command_case_ids)}, window={baseline_window}): {sample}"
                )
    if fail_on_new_lec_runner_command_case_reasons and mode.startswith("LEC"):
        baseline_runner_command_case_reasons_raw = [
            row.get("lec_runner_command_case_reasons") for row in compare_rows
        ]
        if any(raw is not None for raw in baseline_runner_command_case_reasons_raw):
            baseline_runner_command_case_reasons = set()
            for raw in baseline_runner_command_case_reasons_raw:
                if raw is None or raw == "":
                    continue
                for token in raw.split(";"):
                    token = token.strip()
                    if token:
                        baseline_runner_command_case_reasons.add(token)
            current_runner_command_case_reasons = (
                current_lec_runner_command_case_reasons.get(key, set())
            )
            new_runner_command_case_reasons = sorted(
                current_runner_command_case_reasons
                - baseline_runner_command_case_reasons
            )
            if new_runner_command_case_reasons:
                sample = ", ".join(new_runner_command_case_reasons[:2])
                if len(new_runner_command_case_reasons) > 2:
                    sample += ", ..."
                gate_errors.append(
                    f"{suite} {mode}: new LEC runner-command case+reason tuples observed (baseline={len(baseline_runner_command_case_reasons)} current={len(current_runner_command_case_reasons)}, window={baseline_window}): {sample}"
                )
    if fail_on_new_lec_error_bucket_case_ids and mode.startswith("LEC"):
        baseline_error_bucket_case_ids_raw = [
            row.get("lec_error_bucket_case_ids") for row in compare_rows
        ]
        if any(raw is not None for raw in baseline_error_bucket_case_ids_raw):
            baseline_error_bucket_case_ids = set()
            for raw in baseline_error_bucket_case_ids_raw:
                if raw is None or raw == "":
                    continue
                for token in raw.split(";"):
                    token = token.strip()
                    if token:
                        baseline_error_bucket_case_ids.add(token)
            current_error_bucket_case_ids = current_lec_error_bucket_case_ids.get(
                key, set()
            )
            new_error_bucket_case_ids = sorted(
                current_error_bucket_case_ids - baseline_error_bucket_case_ids
            )
            if new_error_bucket_case_ids:
                sample = ", ".join(new_error_bucket_case_ids[:2])
                if len(new_error_bucket_case_ids) > 2:
                    sample += ", ..."
                gate_errors.append(
    ...
    if fail_on_new_lec_semantic_diag_subfamily_case_reasons and mode.startswith("LEC"):
        baseline_subfamily_case_reasons_raw = [
            row.get("lec_semantic_diag_subfamily_case_reasons") for row in compare_rows
        ]
        if any(raw is not None for raw in baseline_subfamily_case_reasons_raw):
            baseline_subfamily_case_reasons = set()
            for raw in baseline_subfamily_case_reasons_raw:
                if raw is None or raw == "":
                    continue
                for token in raw.split(";"):
                    token = token.strip()
                    if token:
                        baseline_subfamily_case_reasons.add(token)
            current_subfamily_case_reasons = (
                current_lec_semantic_diag_subfamily_case_reasons.get(key, set())
            )
            new_subfamily_case_reasons = sorted(
                current_subfamily_case_reasons - baseline_subfamily_case_reasons
            )
            if new_subfamily_case_reasons:
                sample = ", ".join(new_subfamily_case_reasons[:2])
                if len(new_subfamily_case_reasons) > 2:
                    sample += ", ..."
                gate_errors.append(
                    f"{suite} {mode}: new LEC semantic-diag subfamily case+reason tuples observed (baseline={len(baseline_subfamily_case_reasons)} current={len(current_subfamily_case_reasons)}, window={baseline_window}): {sample}"
                )
    if mode.startswith("BMC"):
    ...
    if mode.startswith("LEC"):
        ...
        if fail_on_new_lec_circt_verilog_error_reason_keys:
            baseline_verilog_reason_keys = set()
            for counts in parsed_counts:
                for counter_key in counts.keys():
                    if (
                        counter_key.startswith("lec_circt_verilog_error_reason_")
                        and counter_key.endswith("_cases")
                    ):
                        baseline_verilog_reason_keys.add(counter_key)
            should_enforce_verilog_reason_key_drift = (
                bool(baseline_verilog_reason_keys) or not strict_gate
            )
            if should_enforce_verilog_reason_key_drift:
                current_verilog_reason_keys = {
                    counter_key
                    for counter_key in current_counts.keys()
                    if counter_key.startswith("lec_circt_verilog_error_reason_")
                    and counter_key.endswith("_cases")
                }
                new_verilog_reason_keys = sorted(
                    current_verilog_reason_keys - baseline_verilog_reason_keys
                )
                if new_verilog_reason_keys:
                    sample = ", ".join(new_verilog_reason_keys[:3])
                    if len(new_verilog_reason_keys) > 3:
                        sample += ", ..."
                    gate_errors.append(
        ...
        if fail_on_new_lec_circt_lec_error_reason_keys:
            baseline_lec_error_reason_keys = set()
            for counts in parsed_counts:
                for counter_key in counts.keys():
                    if (
                        counter_key.startswith("lec_circt_lec_error_reason_")
                        and counter_key.endswith("_cases")
                    ):
                        baseline_lec_error_reason_keys.add(counter_key)
            should_enforce_lec_error_reason_key_drift = (
                bool(baseline_lec_error_reason_keys) or not strict_gate
            )
            if should_enforce_lec_error_reason_key_drift:
                current_lec_error_reason_keys = {
                    counter_key
                    for counter_key in current_counts.keys()
                    if counter_key.startswith("lec_circt_lec_error_reason_")
                    and counter_key.endswith("_cases")
                }
                new_lec_error_reason_keys = sorted(
                    current_lec_error_reason_keys - baseline_lec_error_reason_keys
                )
                if new_lec_error_reason_keys:
                    sample = ", ".join(new_lec_error_reason_keys[:3])
                    if len(new_lec_error_reason_keys) > 3:
                        sample += ", ..."
                    gate_errors.append(
    ...
    if suite == "opentitan" and mode == "LEC_STRICT" and opentitan_lec_strict_xprop_counter_prefixes:
        current_counts = parse_result_summary(current_row.get("summary", ""))
        for counter_prefix in opentitan_lec_strict_xprop_counter_prefixes:
            candidate_keys = {
                key for key in current_counts.keys() if key.startswith(counter_prefix)
            }
            for counts in parsed_counts:
                for key in counts.keys():
                    if key.startswith(counter_prefix):
...
if (
    fail_on_bmc_lec_contract_fingerprint_case_id_parity
    or fail_on_new_bmc_lec_contract_fingerprint_case_id_parity
    or fail_on_bmc_lec_contract_fingerprint_case_id_map_unmapped
    or fail_on_new_bmc_lec_contract_fingerprint_case_id_map_unmapped
):
    for bmc_key, bmc_contract_case_ids in sorted(
        current_bmc_contract_fingerprint_case_ids.items()
    ):
        ...
        if not bmc_mode.startswith("BMC"):
            continue
        normalized_bmc_contract_case_ids = set()
        current_identity_fallback_case_ids = set()
        for token in bmc_contract_case_ids:
            ...
            fingerprint = extract_fingerprint_token(token)
            if not case_id or not fingerprint:
                normalized_bmc_contract_case_ids.add(token)
                continue
            mapped_case_id, map_source = map_bmc_to_lec_case_id_with_source(case_id)
            normalized_token = f"{mapped_case_id}::{fingerprint}"
            normalized_bmc_contract_case_ids.add(normalized_token)
            if map_source == "identity":
                current_identity_fallback_case_ids.add(normalized_token)
        normalized_bmc_contract_case_ids.discard("")
        current_identity_fallback_case_ids.discard("")
        ...
        for (lec_suite, lec_mode), lec_case_ids in (
            ...
            if lec_suite != bmc_suite or not lec_mode.startswith("LEC"):
                continue
            current_suite_lec_case_ids.update(lec_case_ids)
        ...
        if (
            fail_on_bmc_lec_contract_fingerprint_case_id_map_unmapped
            and current_identity_missing_case_ids
        ):
        ...
        if (
            (fail_on_new_bmc_lec_contract_fingerprint_case_id_parity and current_missing_case_ids)
            or (
                fail_on_new_bmc_lec_contract_fingerprint_case_id_map_unmapped
                and current_identity_missing_case_ids
            )
            ...
            for (baseline_suite, baseline_mode), baseline_rows in history.items():
                ...
                for row in baseline_compare_rows:
                    ...
                    for token in raw.split(";"):
                        token = token.strip()
                        if token:
                            baseline_suite_lec_case_ids.add(token)
            ...
            if fail_on_new_bmc_lec_contract_fingerprint_case_id_map_unmapped:
                baseline_identity_fallback_case_ids = set()
                for raw in baseline_bmc_raw:
                    if raw is None or raw == "":
                        continue
                    for token in raw.split(";"):
                        ...
                        if not case_id or not fingerprint:
                            continue
                        mapped_case_id, map_source = map_bmc_to_lec_case_id_with_source(
                            case_id
                        )
                        if map_source != "identity":
                            continue
                        baseline_identity_fallback_case_ids.add(
                            f"{mapped_case_id}::{fingerprint}"
                        )
...
if (
    fail_on_mutation_lec_contract_fingerprint_lane_parity
    or fail_on_new_mutation_lec_contract_fingerprint_lane_parity
    or fail_on_mutation_lec_contract_fingerprint_lane_map_unmapped
    or fail_on_new_mutation_lec_contract_fingerprint_lane_map_unmapped
    or fail_on_mutation_lec_contract_fingerprint_lane_map_identity_fallback
    or fail_on_new_mutation_lec_contract_fingerprint_lane_map_identity_fallback
):
    mutation_key = ("mutation-matrix", "PROVENANCE")
    mutation_contract_case_ids = current_mutation_contract_fingerprint_case_ids.get(
        mutation_key, set()
    ...
    if mutation_contract_case_ids and current_lec_contract_fingerprint_case_ids:
        ...
        if mutation_lane_fingerprints and lec_lane_fingerprints:
                        ...
                        if any(raw is not None for raw in baseline_raw):
                            ...
                            for raw in baseline_raw:
                                ...
                                for token in raw.split(";"):
                                    ...
                                    if not lane:
                                        continue
                                    _, map_source = map_mutation_to_lec_lane_id_with_source(
                                        lane
                                    )
                                    if map_source == "identity":
            ...
            if (
                fail_on_mutation_lec_contract_fingerprint_lane_parity
                and current_missing_lane_pairs
            ):
            ...
            if (
                fail_on_new_mutation_lec_contract_fingerprint_lane_parity
                and (current_missing_lane_ids or current_missing_lane_pairs)
            ):
                mutation_history_rows = history.get(mutation_key, [])
                if not mutation_history_rows:
                    if strict_gate and not mutation_gate_enabled:
                    ...
                    if strict_gate and len(mutation_history_rows) < baseline_window:
                        ...
                        if any(raw is not None for raw in baseline_raw):
                            baseline_mutation_lane_fingerprints = {}
                            for raw in baseline_raw:
                                if raw is None or raw == "":
                                    continue
                                for token in raw.split(";"):
                                    ...
                                    if not lane or not fingerprint:
                                        continue
                                    mapped_lane, _ = map_mutation_to_lec_lane_id_with_source(lane)
                                    baseline_mutation_lane_fingerprints.setdefault(
                                        mapped_lane, set()
                                    ).add(fingerprint)
                            ...
                            baseline_missing_lane_ids = sorted(
                                lane
                            ...
                            baseline_missing_lane_pairs = set()
                            for lane, mutation_fingerprints in baseline_mutation_lane_fingerprints.items():
                                lec_fingerprints = baseline_lec_lane_fingerprints.get(lane)
                                if lec_fingerprints is None:
                                    continue
                                for fingerprint in sorted(mutation_fingerprints - lec_fingerprints):
