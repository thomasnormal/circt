...
ROOT_DIR="$(checks_repo_root "$SCRIPT_DIR")"
RUNNER_PY="$ROOT_DIR/utils/run_cvdp_cocotb_runner.py"
...
if status != "COCOTB_FAIL":
    raise SystemExit(f"expected COCOTB_FAIL for harness python errors, got {status}")
PY
