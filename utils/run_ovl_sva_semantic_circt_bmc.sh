...
run_case_mode() {
  ...
  local is_fail_gap=0
  if [[ "$mode" == "fail" && "$known_gap" == "1" ]]; then
    is_fail_gap=1
  fi

  if [[ "$got_result" != "$expect_result" ]]; then
    if [[ "$is_fail_gap" == "1" ]]; then
      xfail=$((xfail + 1))
      [[ -n "$OUT" ]] && printf 'XFAIL(%s): %s [%s expected=%s got=%s]\n' "$mode" "$case_id" "KNOWN_GAP" "$expect_result" "${got_result:-NONE}" >> "$OUT"
      return
    fi
    failures=$((failures + 1))
    [[ -n "$OUT" ]] && printf 'FAIL(%s): %s [%s expected=%s got=%s]\n' "$mode" "$case_id" "UNEXPECTED_RESULT" "$expect_result" "${got_result:-NONE}" >> "$OUT"
    return
  fi

  if [[ "$is_fail_gap" == "1" ]]; then
    xpass=$((xpass + 1))
    [[ -n "$OUT" ]] && printf 'XPASS(%s): %s\n' "$mode" "$case_id" >> "$OUT"
    return
  fi

  [[ -n "$OUT" ]] && printf 'PASS(%s): %s\n' "$mode" "$case_id" >> "$OUT"
}
