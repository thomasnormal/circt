...
append_lec_resolved_contract() {
  ...
  local payload
  payload="${contract_source}"$'\x1f'"${backend_mode}"$'\x1f'"${CIRCT_TIMEOUT_SECS}"$'\x1f'"${run_mode}"$'\x1f'"${LEC_ASSUME_KNOWN_INPUTS}"$'\x1f'"${LEC_ACCEPT_XPROP_ONLY}"$'\x1f'"${CIRCT_LEC_ARGS}"
  local fingerprint
  ...
  printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
    ...
    "$LEC_ACCEPT_XPROP_ONLY" "$CIRCT_LEC_ARGS" "$fingerprint" \
    >> "$LEC_RESOLVED_CONTRACTS_OUT"
}
...
for sv in "$YOSYS_SVA_DIR"/*.sv; do
  ...
  total=$((total + 1))
  append_lec_resolved_contract "$base" "$sv" "lec"
...
"
  ...
  if run_limited "$opt_bin" "${opt_args[@]}" > "$opt_mlir" 2> "$opt_log"; then
    ...
    if grep -Eiq "failed to run command .*(text file busy|permission denied)" "$opt_log"; then
      ...
      append_lec_launch_event \
        "RETRY" "$base" "$sv" "opt" "$opt_bin" "$launch_reason" \
        "1" "$CIRCT_RETRY_TEXT_FILE_BUSY_DELAY_SECS" "$opt_status" ""
      sleep "$CIRCT_RETRY_TEXT_FILE_BUSY_DELAY_SECS"
      original_opt_tool="$opt_bin"
