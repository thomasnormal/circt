...
run_case() {
  ...
  if [[ "$run_sim_only_case" == "1" ]]; then
    ...
    local sim_top_name=""
    local sim_cycles=""
    local sim_toggle_steps="$SIM_ONLY_TOGGLE_STEPS"
    sim_top_name="$(awk '
    ...
    ' "$ys_file")"
    if [[ -n "$sim_cycles" && "$sim_cycles" =~ ^[0-9]+$ && "$sim_cycles" -gt 0 ]]; then
      sim_toggle_steps=$((sim_cycles * 2))
    fi
    clock_name="$(awk '
      /^[[:space:]]*sim([[:space:]]|$)/ {
        for (i = 1; i <= NF; ++i) {
