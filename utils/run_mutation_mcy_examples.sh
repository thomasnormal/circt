...
Options:
  ...
  --history-detected-aggregation-mode MODE
                           Detected-metric aggregation override for history
                           baselines: inherit|percentile|ewma|max
                           (default: inherit)
  --history-relevant-aggregation-mode MODE
                           Relevant-metric aggregation override for history
                           baselines: inherit|percentile|ewma|max
                           (default: inherit)
  --history-coverage-aggregation-mode MODE
                           Coverage-metric aggregation override for history
                           baselines: inherit|percentile|ewma|max
                           (default: inherit)
  --history-errors-aggregation-mode MODE
                           Error-metric aggregation override for history
                           baselines: inherit|percentile|ewma|max
                           (default: inherit)
...
MUTATION_MCY_LIB_DIR="${SCRIPT_DIR}/mutation_mcy/lib"

# Shared mutation helpers keep native-plan and worker logic centralized.
source "${MUTATION_MCY_LIB_DIR}/native_mutation_plan.sh"
source "${MUTATION_MCY_LIB_DIR}/worker.sh"
...
HISTORY_DETECTED_AGGREGATION_MODE="inherit"
HISTORY_RELEVANT_AGGREGATION_MODE="inherit"
HISTORY_COVERAGE_AGGREGATION_MODE="inherit"
HISTORY_ERRORS_AGGREGATION_MODE="inherit"
...
declare -A EXAMPLE_TO_MAX_DETECTED_DROP=()
...
is_valid_native_mutation_op_token() {
  case "$1" in
    EQ_TO_NEQ|NEQ_TO_EQ|LT_TO_LE|GT_TO_GE|LE_TO_LT|GE_TO_GT|LT_TO_GT|GT_TO_LT|LE_TO_GE|GE_TO_LE|\
    CASEEQ_TO_EQ|CASENEQ_TO_NEQ|EQ_TO_CASEEQ|NEQ_TO_CASENEQ|\
    SIGNED_TO_UNSIGNED|UNSIGNED_TO_SIGNED|\
    AND_TO_OR|OR_TO_AND|LAND_TO_BAND|LOR_TO_BOR|XOR_TO_OR|XOR_TO_XNOR|XNOR_TO_XOR|REDAND_TO_REDOR|REDOR_TO_REDAND|REDXOR_TO_REDXNOR|REDXNOR_TO_REDXOR|\
    BAND_TO_BOR|BOR_TO_BAND|BAND_TO_LAND|BOR_TO_LOR|BAND_EQ_TO_BOR_EQ|BOR_EQ_TO_BAND_EQ|UNARY_NOT_DROP|UNARY_BNOT_DROP|\
    BA_TO_NBA|NBA_TO_BA|ASSIGN_RHS_TO_CONST0|ASSIGN_RHS_TO_CONST1|ASSIGN_RHS_TO_LHS|ASSIGN_RHS_OR_LHS|ASSIGN_RHS_AND_LHS|ASSIGN_RHS_XOR_LHS|ASSIGN_RHS_ADD_LHS|ASSIGN_RHS_SUB_LHS|ASSIGN_RHS_INVERT|ASSIGN_RHS_PLUS_ONE|ASSIGN_RHS_MINUS_ONE|ASSIGN_RHS_NEGATE|ASSIGN_RHS_SHL_ONE|ASSIGN_RHS_SHR_ONE|ASSIGN_RHS_ASHR_ONE|POSEDGE_TO_NEGEDGE|NEGEDGE_TO_POSEDGE|IF_COND_NEGATE|RESET_COND_NEGATE|RESET_COND_TRUE|RESET_COND_FALSE|IF_COND_TRUE|IF_COND_FALSE|WAIT_COND_NEGATE|WAIT_COND_TRUE|WAIT_COND_FALSE|WHILE_COND_NEGATE|WHILE_COND_TRUE|WHILE_COND_FALSE|FOR_COND_NEGATE|FOR_COND_TRUE|FOR_COND_FALSE|IF_ELSE_SWAP_ARMS|REPEAT_COUNT_PLUS_ONE|REPEAT_COUNT_MINUS_ONE|DELAY_PLUS_ONE|DELAY_MINUS_ONE|CASE_ITEM_SWAP_ARMS|CASE_TO_CASEZ|CASEZ_TO_CASE|CASE_TO_CASEX|CASEX_TO_CASE|CASEZ_TO_CASEX|CASEX_TO_CASEZ|MUX_SWAP_ARMS|MUX_FORCE_TRUE|MUX_FORCE_FALSE|\
    CONST0_TO_1|CONST1_TO_0|CONST_PLUS_ONE|CONST_MINUS_ONE|CMP_CONST_PLUS_ONE|CMP_CONST_MINUS_ONE|\
    ADD_TO_SUB|SUB_TO_ADD|MUL_TO_ADD|ADD_TO_MUL|DIV_TO_MUL|MUL_TO_DIV|MOD_TO_DIV|DIV_TO_MOD|UNARY_MINUS_DROP|INC_TO_DEC|DEC_TO_INC|PLUS_EQ_TO_MINUS_EQ|MINUS_EQ_TO_PLUS_EQ|MUL_EQ_TO_DIV_EQ|DIV_EQ_TO_MUL_EQ|MOD_EQ_TO_DIV_EQ|DIV_EQ_TO_MOD_EQ|SHL_EQ_TO_SHR_EQ|SHR_EQ_TO_SHL_EQ|SHR_EQ_TO_ASHR_EQ|ASHR_EQ_TO_SHR_EQ|\
...
validate_native_real_harness_args_spec() {
  ...
  if ! bash -n -c "set -- ${spec}" >/dev/null 2>&1; then
    echo "${context}: native real harness args contain invalid shell quoting" >&2
    return 1
  fi
  return 0
}
...
resolved_history_metric_aggregation_mode() {
  local metric="$1"
  local configured="inherit"
  case "$metric" in
    detected) configured="$HISTORY_DETECTED_AGGREGATION_MODE" ;;
    relevant) configured="$HISTORY_RELEVANT_AGGREGATION_MODE" ;;
    coverage) configured="$HISTORY_COVERAGE_AGGREGATION_MODE" ;;
    errors) configured="$HISTORY_ERRORS_AGGREGATION_MODE" ;;
    *) configured="inherit" ;;
  esac

  if [[ "$configured" == "inherit" ]]; then
    printf '%s\n' "$HISTORY_AGGREGATION_MODE"
  else
    printf '%s\n' "$configured"
  fi
}
...
compute_example_metric_percentile_from_histories() {
  ...
  local metric_name=""
  case "$field_index" in
    4) metric_name="detected" ;;
    5) metric_name="relevant" ;;
    6) metric_name="coverage" ;;
    7) metric_name="errors" ;;
    *) metric_name="" ;;
  esac

  local effective_mode="$HISTORY_AGGREGATION_MODE"
  if [[ -n "$metric_name" ]]; then
    effective_mode="$(resolved_history_metric_aggregation_mode "$metric_name")"
  fi

  if [[ "$effective_mode" == "ewma" ]]; then
    if [[ "$value_kind" == "int" ]]; then
  ...
  elif [[ "$effective_mode" == "max" ]]; then
    if [[ "$value_kind" == "int" ]]; then
      value="$(max_int_from_file "$tmp_values_file")"
    else
      value="$(max_decimal_from_file "$tmp_values_file")"
    fi
  else
    if [[ "$value_kind" == "int" ]]; then
...
# run_example_worker is sourced from mutation_mcy/lib/worker.sh.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --history-ewma-alpha)
      ...
      shift 2
      ;;
    --history-detected-aggregation-mode)
      HISTORY_DETECTED_AGGREGATION_MODE="$2"
      shift 2
      ;;
    --history-relevant-aggregation-mode)
      HISTORY_RELEVANT_AGGREGATION_MODE="$2"
      shift 2
      ;;
    --history-coverage-aggregation-mode)
      HISTORY_COVERAGE_AGGREGATION_MODE="$2"
      shift 2
      ;;
    --history-errors-aggregation-mode)
      HISTORY_ERRORS_AGGREGATION_MODE="$2"
      shift 2
      ;;
    --update-baseline)
      UPDATE_BASELINE=1
