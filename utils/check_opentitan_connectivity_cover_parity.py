#!/usr/bin/env python3
"""Check rule-level OpenTitan connectivity cover-counter parity between BMC and LEC.
...
Only cover counters present in both inputs are compared. This allows the check
to run before all lanes emit full cover schemas.
"""
...
def read_status_summary(
    ...
    with path.open(encoding="utf-8", newline="") as handle:
        ...
        if "rule_id" not in reader.fieldnames:
            fail(
                f"{lane_name} status summary missing required column ['rule_id']: {path} "
                f"(found: {reader.fieldnames})"
            )
        available_cover_fields = tuple(
            field for field in COVER_FIELDS if field in reader.fieldnames
        )
        ...
        for idx, row in enumerate(reader, start=2):
            ...
            out[rule_id] = {
                field: (row.get(field) or "").strip() for field in available_cover_fields
            }
    return out, available_cover_fields
...
def main() -> None:
    ...
    bmc, bmc_fields = read_status_summary(bmc_path, "connectivity BMC")
    lec, lec_fields = read_status_summary(lec_path, "connectivity LEC")
    shared_fields = tuple(field for field in COVER_FIELDS if field in set(bmc_fields).intersection(lec_fields))
    ...
    if non_allowlisted_rows:
        ...
        if len(non_allowlisted_rows) > 6:
            sample += ", ..."
        message = (
            "opentitan connectivity cover parity mismatches detected: "
    ...
    print(
        "opentitan connectivity cover parity check passed: "
        f"rules_bmc={len(bmc)} rules_lec={len(lec)} "
        f"shared_cover_fields={len(shared_fields)}",
        file=sys.stderr,
    )
