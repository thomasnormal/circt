...
### Problem
1. Sequence match-item `$fatal` calls were still ignored with remarks:
   - `ignoring system subroutine '$fatal' in assertion match items`
2. This left fatal subroutine parity incomplete in match-item side-effect
   lowering.

### Fix
1. Updated `lib/Conversion/ImportVerilog/AssertionExpr.cpp` in
   `handleMatchItems` system-call handling:
   - added `$fatal` support in match-item call lowering.
   - lowered `$fatal` to:
     - `moore.builtin.severity fatal`
     - `moore.builtin.finish 1`
2. Added regression:
   - `test/Conversion/ImportVerilog/sva-sequence-match-item-fatal-subroutine.sv`
...
### Problem
1. File-oriented sequence match-item system subroutines were still ignored with
   remarks:
   - `$fdisplay`
   - `$fwrite`
   - `$fstrobe`
   - `$fmonitor`
2. This left side-effect parity gaps for common logging/debug file tasks.

### Fix
1. Updated `lib/Conversion/ImportVerilog/AssertionExpr.cpp` in
   `handleMatchItems` system-call handling:
   - added match-item lowering for `$fdisplay*` / `$fwrite*` to
     `moore.builtin.fwrite`.
   - added match-item lowering for `$fstrobe*` to
     `moore.builtin.fstrobe`.
   - added match-item lowering for `$fmonitor*` to
     `moore.builtin.fmonitor`.
   - added file descriptor conversion path to i32 for these tasks.
2. Added regression:
   - `test/Conversion/ImportVerilog/sva-sequence-match-item-file-subroutine.sv`
...
### Problem
1. Sequence match-item file-control tasks were still ignored with remarks:
   - `$fflush`
   - `$fclose`
2. This left file-control side-effect parity incomplete in match-item
   subroutine lowering.

### Fix
1. Updated `lib/Conversion/ImportVerilog/AssertionExpr.cpp` in
   `handleMatchItems` system-call handling:
   - added `$fflush` lowering to `moore.builtin.fflush`.
   - added `$fclose` lowering to `moore.builtin.fclose`.
   - reused match-item fd conversion path to i32.
2. Added regression:
...
### Problem
1. Sequence match-item control tasks were still ignored with remarks:
   - `$finish`
   - `$stop`
   - `$dumpvars`
2. This left control-task side-effect parity incomplete for match-item
   subroutines.

### Fix
1. Updated `lib/Conversion/ImportVerilog/AssertionExpr.cpp` in
   `handleMatchItems` system-call handling:
   - added `$stop` lowering to `moore.builtin.stop`.
   - added `$finish` lowering to `moore.builtin.finish 0`.
   - added `$dumpvars` lowering to display marker carrying
     `circt.dumpvars` attribute.
2. Added regression:
   - `test/Conversion/ImportVerilog/sva-sequence-match-item-control-subroutine.sv`
...
### Problem
1. Sequence match-item `$timeformat` and dump-control tasks still emitted
   ignored-system-subroutine remarks:
   - `$timeformat`
   - `$dumpoff/$dumpon/$dumpflush/$dumpall` (and related dump-controls)
2. This left parser-reachable match-item task parity incomplete and noisy.

### Fix
1. Updated `lib/Conversion/ImportVerilog/AssertionExpr.cpp` in
   `handleMatchItems` system-call handling:
   - added `$timeformat` lowering to `moore.builtin.timeformat` with
     argument/default handling aligned to statement-lowering shape.
   - added recognition of dump-control no-op family in match-items:
     `$dumplimit`, `$dumpoff`, `$dumpon`, `$dumpflush`, `$dumpall`,
     `$dumpports`, `$dumpportslimit`, `$dumpportsoff`, `$dumpportson`,
     `$dumpportsflush`, `$dumpportsall`.
2. Added regression:
   - `test/Conversion/ImportVerilog/sva-sequence-match-item-timeformat-dumpcontrol-subroutine.sv`
...
### Problem
1. Sequence match-item `$showvars` still emitted an
   ignored-system-subroutine remark and produced no side-effect op.
2. This left a parser-reachable debug-task gap in match-item parity.

### Fix
1. Updated `lib/Conversion/ImportVerilog/AssertionExpr.cpp` in
   `handleMatchItems` system-call handling:
   - added `$showvars` lowering for match items.
   - for each argument, collects a variable-name fragment (when available),
     converts the value to a simple bit-vector, formats decimal output, and
     emits newline-delimited fragments.
   - concatenates fragments into a single message and emits
     `moore.builtin.display`.
2. Added regression:
   - `test/Conversion/ImportVerilog/sva-sequence-match-item-debug-subroutine.sv`
...
### Problem
1. The following statement-supported tasks were still ignored in sequence
   match-items:
   - `$showscopes`, `$input`, `$key`, `$nokey`, `$log`, `$nolog`
   - `$save`, `$restart`, `$incsave`, `$reset`
2. This left parser-reachable match-item parity gaps and noisy ignore remarks.

### Fix
1. Updated `lib/Conversion/ImportVerilog/AssertionExpr.cpp` in
   `handleMatchItems` system-call handling:
   - recognized debug/interactive family as no-op controls in match-items:
     `$showscopes`, `$input`, `$key`, `$nokey`, `$log`, `$nolog`.
   - recognized checkpoint/restart family and emitted statement-parity
     warnings:
     - `$save`, `$restart`, `$incsave`, `$reset`
     - warning text: `... is not supported in circt-sim (checkpoint/restart not implemented)`.
2. Added regression:
...
### Problem
1. `$scope` and `$list` were parser-reachable in sequence match-items but still
   emitted ignored-subroutine remarks.
2. This left a debug-task parity gap relative to statement-level handling.

### Fix
1. Updated `lib/Conversion/ImportVerilog/AssertionExpr.cpp` in
   `handleMatchItems` system-call handling:
   - added `$scope` and `$list` to the recognized debug no-op family in
     match-items.
2. Added regression:
   - `test/Conversion/ImportVerilog/sva-sequence-match-item-scope-list-subroutine.sv`
3. Updated docs/changelog:
   - `PROJECT_SVA.md`
   - `CHANGELOG.md`
...
### Problem
1. Parser-reachable match-item tasks were still ignored with remarks:
   - `$set_coverage_db_name`
   - `$load_coverage_db`
   - `$sdf_annotate`
   - `$static_assert`
2. This left a parity gap vs statement-level handling and produced avoidable
   ignored-subroutine noise.

### Fix
1. Updated `lib/Conversion/ImportVerilog/AssertionExpr.cpp` in
   `handleMatchItems` system-call handling:
   - added no-op handling for coverage persistence tasks:
     `$set_coverage_db_name`, `$load_coverage_db`.
   - added warning handling for `$sdf_annotate`:
     `... is not supported in circt-sim (SDF timing annotation not implemented)`.
   - added compile-time no-op handling for `$static_assert`.
2. Added regression:
...
### Problem
1. `$stacktrace` was parser-reachable as a value-returning system function in
   sequence match-item assignment RHS (for `string` locals), but ImportVerilog
   rejected it with:
   - `unsupported system call '$stacktrace'`
2. This blocked match-item forms like:
   - `string st; (1, st = $stacktrace) ##1 a;`

### Fix
1. Updated `lib/Conversion/ImportVerilog/Expressions.cpp`:
   - added `$stacktrace` handling in system-call expression lowering.
   - computes scope ancestry from `currentScope` up to root / compilation unit.
   - materializes a Moore `string` value via:
     - `moore.constant_string` (packed int bytes)
     - `moore.int_to_string` (dynamic string)
   - wired both explicit call-path handling and arity-0 conversion path.
2. Added regression:
...
### Problem
1. Match-item display/file-display and strobe/monitor families were lowered as
   marker-only side effects:
   - `"$display\\n"`, `"$fdisplay\\n"`, etc.
2. Real format arguments were discarded, so value reads/formatting did not
   mirror statement-level behavior.

### Fix
1. Updated `lib/Conversion/ImportVerilog/AssertionExpr.cpp` in
   `AssertionExprVisitor::handleMatchItems` system-call handling:
   - switched marker-literal lowering to `context.convertFormatString` for:
     - `$display/$write` (+ `b/o/h`)
     - `$strobe` (+ `b/o/h`)
     - `$monitor` (+ `b/o/h`)
     - `$fdisplay/$fwrite` (+ `b/o/h`)
     - `$fstrobe` (+ `b/o/h`)
     - `$fmonitor` (+ `b/o/h`)
   - preserved existing side-effect ops while now carrying actual formatted
     payloads.
2. Added regression:
