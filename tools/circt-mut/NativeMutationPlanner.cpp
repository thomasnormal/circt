...
static void
collectWhileConditionSites(StringRef text, ArrayRef<uint8_t> codeMask,
                           SmallVectorImpl<SiteInfo> &sites) {
  for (size_t i = 0, e = text.size(); i + 4 < e; ++i) {
    if (!matchKeywordTokenAt(text, codeMask, i, "while"))
      continue;

    size_t openPos = i + strlen("while");
    while (openPos < e &&
           (!isCodeAt(codeMask, openPos) ||
            std::isspace(static_cast<unsigned char>(text[openPos]))))
      ++openPos;
...
static void collectSitesForOp(StringRef designText, StringRef op,
  ...
  if (op == "WHILE_COND_NEGATE" || op == "WHILE_COND_TRUE" ||
      op == "WHILE_COND_FALSE") {
...
static bool
findWhileConditionBoundsAtSite(StringRef text, ArrayRef<uint8_t> codeMask,
                               size_t whilePos, size_t &condOpen,
                               size_t &condClose) {
  if (whilePos == StringRef::npos ||
      whilePos + strlen("while") > text.size() ||
      !matchKeywordTokenAt(text, codeMask, whilePos, "while"))
    return false;

  size_t openPos = whilePos + strlen("while");
  while (openPos < text.size() &&
         (!isCodeAt(codeMask, openPos) ||
          std::isspace(static_cast<unsigned char>(text[openPos]))))
    ++openPos;
  ...
  if (op == "IF_COND_NEGATE" || op == "RESET_COND_NEGATE" ||
    ...
    if (op == "WAIT_COND_NEGATE" || op == "WAIT_COND_TRUE" ||
      ...
      if (!findWaitConditionBoundsAtSite(text, codeMask, pos, condOpen,
                                         condClose))
        return false;
    } else if (op == "WHILE_COND_NEGATE" || op == "WHILE_COND_TRUE" ||
               op == "WHILE_COND_FALSE") {
